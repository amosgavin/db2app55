...
function Box(objID,W,H,inner,footerInner){
	...
	if(!$(objID)){
		...
		if(W==undefined||W<200||W==""){W=200}
		if(H==undefined||H<60||H==""){H=60}
		if(inner==undefined || inner=="" ){inner=""}
		if(footerInner==undefined || footerInner=="" ){footerInner="www.shuxu.com &copy; 2009"}
		...
		$(closeBtn).onclick = function(){Close(objID)};
		if($(cancelBtn)){$(cancelBtn).onclick = function(){Close(objID)}}
