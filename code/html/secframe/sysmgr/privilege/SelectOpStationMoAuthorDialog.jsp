...
<script type="text/javascript">
	...
	{
		selOpStationId = window.dialogArguments;
		if(selOpStationId != null)
		{
			closeTree();
			loadFuncMoTree(selOpStationId);
		}
	}
	/**
	 * 收缩菜单MO树
	...
	function saveMoStationAuthor()
	{
		...
		// 保存
			var param = "&stationId="+stId+"&moIds="+moIds+"&moNames="+moNames;
			var msg = PostInfo("<%=request.getContextPath()%>/business/com.ai.secframe.sysmgr.web.PrivilegeAction?action=saveMoOpstationAuthor"+param);
			if(msg != null)
			{
		    ...
		    else
		    {
		    	...
		    	alert(g_I18NMessage("secframe_common", "sec_common_savefail_msg", retMsg));
		    }
			}   	
	}
	/**
	...
	{
		var funcMoTree = g_DBTreeNewManager.get("funcMoTree");
		funcMoTree.refresh(rootId, -1, "isopstation=Y&stationId="+stId);
		setStatus(rootId);		
	}
