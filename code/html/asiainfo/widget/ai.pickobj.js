 ...
 getSelValue:function(){
  ...
  OKClick : function(){
  	if(this.bindCmp){
  		...
  		this.bindCmp.setValue(result.rawvalue);
     ...
     if(subtype=='grid') this.initGridControl(_columns)
	   ...
	   this.selGrid = new Asiainfo.widget.Grid({
	      ...
	      width:250,
				subtype:'grid',
				stripeRows:true,
				columns:[{ header: this.keyfield, dataIndex:this.keyfield, width:100 },
				         {header: this.valfield, dataIndex: this.valfield, width:150 }
				        ],
				layout:'fit'
		}).control; 
