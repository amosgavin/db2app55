...
Asiainfo.widget.TreeGrid =function (config) {
	baseFun.loadScript('../asiainfo/widget/ai.treegridex/css/TreeGrid.css','css');
	//baseFun.loadScript('../asiainfo/widget/ai.treegridex/css/TreeGridLevels.css','css');
        baseFun.loadScript('../asiainfo/widget/ai.treegridex/TreeGrid.js'); 
	this.cfg = config || {};
	if (config.cfgcode)
...
Asiainfo.widget.TreeGrid.prototype = {
   ...
   create : function() {
	...
	this.record = Ext.data.Record.create(fields);
      ...
      sortInfo:{field:this.cfg.pkeyname,direction:'ASC'},
　　   autoLoad : true    
　　  ,reader: new Ext.data.JsonReader({root:'root',  id: this.cfg.keyname}, this.record)
    ...
    this.store.on('remove',function(Store,record,Number){
      alert('delete:'+Ext.encode(record.data))
    });
    this.AddChild();
