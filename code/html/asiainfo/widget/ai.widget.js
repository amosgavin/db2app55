 ...
 return {
	   ...
	   for(var i=0;i<=objs.length;i++){
	    ...
	    function onOKSave(){
	     ...
	     if(!fmwin){
	     	    ...
	     	    fmwin = new Ext.Window({id:'win_'+formid, title: "信息", width:winWidth, height:winHeight, layout: 'fit', plain:true,
                      modal: true, bodyStyle:'padding:1px;', buttonAlign:'center',closable:false,
                      items: [fm],   
   ...
   getField:function (c,store){
	     ...
	     if(c.xtype=='label' || c.xtype=='标签')
	     	...
	     	baseFun.loadScript('../asiainfo/form/ComboBoxTree.js');
	    	...
	    	c.fileUpload=true ;
	    	baseFun.loadScript('../asiainfo/form/FileUploadField.js');
	        ...
	        fd=Asiainfo.widget.getMuliseleField(c)
	    	 ...
	    	 fd=new Ext.form.Checkbox(c)
	   		...
	   		baseFun.loadScript('../asiainfo/form/CheckboxGroup.js');
	   		var items=[];
	   		if(!c.columns || c.columns > c.boxLabels.length) c.columns = c.boxLabels.length;
	    	...
	    	searchWin.AddTrigfield(fd,c.storesql,null,null,c.storeId);
	        if(c.parent){
	    	fd.on('focus',function(){
	    		var cmp=Ext.getCmp(c.parent);
	    		
	    		fd.store.templateSQL='select 111 values1,222 values2 from md.metauser';
	    		 
	    	   });
	        }
	    } 
	    else if(c.xtype=='pick'){
	    	baseFun.loadScript('../asiainfo/widget/ai.pickobj.js');
	    	c.triggerClass= 'x-form-search-trigger';
	    	if(!c.onTriggerClick){
   ...
   RefreshView : function(objcode){
	...
	getFileField:function(c){
	   baseFun.loadScript('../asiainfo/form/UploadDialog/Ext.ux.UploadDialog.js');
	   baseFun.loadScript('../asiainfo/form/UploadDialog/css/Ext.ux.UploadDialog.css','css');
	   baseFun.loadScript('../asiainfo/form/FileUploadField.js');
	   return new Ext.form.FileField( c ); 
	},
	getMuliseleField:function(c){
		 
	   baseFun.loadScript('../asiainfo/widget/MultiSelectField.js');
  ...
  expData:function(gd_result){
	...
	Ext.util.FormSubmit('/'+contextPath+'/pst/export',{
		sql:exportSql,
