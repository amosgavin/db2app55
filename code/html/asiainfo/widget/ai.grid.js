...
Asiainfo.widget.Grid.prototype={
    ...
    for(var i=0;i<this.cfg.columns.length;i++){
    	...
    	for(var j=0;j<this.store.recordFields.length;j++){
  		  ...
  		  if(this.cfg.columns[i].dataIndex==this.store.recordFields[j].name && this.store.recordFields[j].type=='date'){
  		    ...
  		    if(this.cfg.subtype=='edgrid') this.cfg.columns[i].editor=new  Ext.form.DateField({format:'Y-m-d'})
  		  }
  ...
  if(this.cfg.showcheck){
	...
	if(this.cfg.subtype=='ghgrid'){
		baseFun.loadScript('../asiainfo/grid/GroupHeaderPlugin.css','css');
		baseFun.loadScript('../asiainfo/grid/GroupHeaderPlugin.js');
	...
	else if(this.cfg.subtype=='expandgrid'){
	...
	else if(this.cfg.subtype=='groupgrid' || this.cfg.subtype=='edgroupgrid' ){
