...
var fd_stepdb = new Ext.form.ComboBox({
 ...
 var ds_fd_stepfunc= new Ext.data.SimpleStore({
      		fields:['ID','VALUE'],
      		data:[['1','url'],['2','showsql'],['3','remark'],['4','olap']]
      	});
...
var f_stepinfo = new Ext.form.FormPanel({width:'90%',style : 'height: 90%;',layout: 'form', 
   items:[{layout:'column', border:false,
         ...
         fd_stepfunc,fd_stepname,fd_stepscript,fd_steppara]
});
...
var ds_proc = new Asiainfo.data.AsiaInfoJsonStore({
			table:schema+'.DQCHECKMODEL',
			root:'root',
			sql:"select PROC_NAME,PROCCNNAME,CYCLETYPE,TOPICNAME,REMARK,PROCTYPE,RUNPARA,DEVELOPER,EFF_DATE,CREATER, STATE, STATE_DATE, CURDUTYER,Value(VERSEQ,0) VERSEQ,XML from "+schema+".DQCHECKMODEL where PROC_NAME='"+procName+"'",
			initUrl:'/'+contextUrl+'/newrecordService',
			url:'/'+contextUrl+'/newrecordService',
			dataSource:DBNAME,
			loadDataWhenInit:true,
...
function main() {
    ...
    var btnSaveNode = new Ext.Button({
        ...
        handler: function() {
            ...
            store = new Asiainfo.data.AsiaInfoJsonStore({
                table: 'MD.DQCHECKSTEP',
                root: 'root',
                sql: "select PROC_NAME, STEP_SEQ, NEXT_STEP, STEP_NAME, STEP_TYPE, STEP_CODE, SQL_TEXT, DBNAME from MD.DQCHECKSTEP where PROC_NAME='" + procName + "'",
                initUrl: '/' + contextUrl + '/newrecordService',
                url: '/' + contextUrl + '/newrecordService',
                key: 'PROC_NAME'
            });
            ...
            store = new Asiainfo.data.AsiaInfoJsonStore({
                table: 'MD.DQCHECKSTEP',
                root: 'root',
                sql: 'select PROC_NAME, STEP_SEQ, NEXT_STEP, STEP_NAME, STEP_TYPE, STEP_CODE, SQL_TEXT, DBNAME,OBJX,OBJY,AREACODE, NEXTCOND from MD.DQCHECKSTEP where 1=2',
                initUrl: '/' + contextUrl + '/newrecordService',
                url: '/' + contextUrl + '/newrecordService',
                key: 'PROC_NAME'
            });
    ...
    var btnCompare = new Ext.Button({
        ...
        handler: function() {
            Asiainfo.addTabSheet('syn' + procName, '程序发布(' + procName + ')', '/devmgr/ObjCompare.html?OBJTYPE=PROC&OBJNAME=' + procName + '&TARDB=&SOURDB=METADB&ONLYDIFF=F');
        }
    });
    var btnHis = new Ext.Button({
        ...
        handler: function() {
            Asiainfo.addTabSheet('his' + procName, '程序版本(' + procName + ')', '/devmgr/ObjHis.html?OBJTYPE=PROC&OBJNAME=' + procName);
        }
    });
    ...
    var btnProcRela = new Ext.Button({
        ...
        handler: function() {
          Asiainfo.addTabSheet(procName+'eeproc',procName+'程序影响分析','./asiainfo/ProcGraph/rela.html?annyType=ETL&annyDirection=All&level=3&relaName='+procName);
      ...
      var btnProcGerDoc = new Ext.Button({
        ...
        handler: function() {
          window.open('../../devmgr/GerDoc.html?OBJTYPE=ETL&OBJNAME='+procName, '生成程序文档');
    ...
    function getNextCellId(cell, store) {
        var nextid;
        var isEnd = true;
        var parent = graph.getDefaultParent();
        var childCount = graph.getModel().getChildCount(parent);
        ...
        for (var i = 0; i < childCount; i++) {
            ...
            if (child.isEdge() && child.getSource() && child.getSource().getId() == cell.getId()) {
            	      var nextCell = child.getTarget();
            	      rec = store.newRecord();
                    rec.set('PROC_NAME', procName);
                    rec.set('STEP_SEQ', cell.getId());
                     
                    rec.set('OBJX',cell.geometry.x);
	                  rec.set('OBJY',cell.geometry.y);
                    rec.set('STEP_NAME', cell.getValue()||"");
                    //rec.set('DBNAME', cell.getobjType()||"");
                    rec.set('NEXT_STEP', nextCell.getId()||"");
                   // rec.set('NEXTCOND', child.getLinkType()||-1);
                    
                    rec.set('STEP_TYPE', cell.getFn() == null ? -1 : cell.getFn());
                    var sqltext = cell.getScript() + '';
                    sqltext = sqltext.replace(/\r\n/gi, " ").replace(/\t/gi, " ").replace(/^\s+|\s+$/, '');
                    rec.set('SQL_TEXT', cell.getScript() == null ? '': sqltext)            
                    store.add(rec);
                    bAddOnece=true;
            }
        };
        if(!bAddOnece){
        	    rec = store.newRecord();
                    ...
                    rec.set('DBNAME', cell.getobjType());
                    rec.set('NEXT_STEP', -1);
                    rec.set('NEXTCOND', child.getLinkType());
                    rec.set('OBJX',cell.geometry.x);
	                  rec.set('OBJY',cell.geometry.y);
