            ...
            menu:
            {
		        ...
		        },
		        {
		            ...
		            {             
									...
									var btn_16= new Ext.Button({
									     text: 'Import',
									     handler:function(button,event){
									     	var xml=fd_8.getValue();
												if (xml != null && xml.length > 0)
							        		{
							        			var doc = mxUtils.parseXml(xml); 
							        			var dec = new mxCodec(doc); 
									...
									var fd_8 = new Ext.form.TextArea({
									    fieldLabel:'Content',
									    name:'oText',
									    id:'oText',
									    width:290,
                  ...
                  var w=new Ext.Window({
					           width:300,
					           height:200,
					           title:'Pase xml text into the textarea please' ,
					           items:[fd_8],
					           buttons:[btn_16],
					           buttonAlign:'center',
					           modal:true
		        ...
		        },
		        {
                 ...
                 var enc = new mxCodec(mxUtils.createXmlDocument());
						     var node = enc.encode(graph.getModel());
                 var win=window.open();
                 win.document.write(mxUtils.getPrettyXml(node));
                
                 win.document.execCommand('Saveas', 'c:\\'+procName+'.xml');
                 //win.close();	        		
		            }
		        },
		        '-',
