...
var ds_flow = new Asiainfo.data.AsiaInfoJsonStore({
			root:'root',
			sql:"select FLOWCODE,FLOWNAME,XML,VERSEQ,CURDUTYER,EFF_DATE,CREATER,STATE,STATE_DATE from MD.TRANSFLOW where FLOWCODE='"+FLOWCODE+"'",
			initUrl:'/'+contextUrl+'/newrecordService',
			url:'/'+contextUrl+'/newrecordService',
			loadDataWhenInit:true,
			table:"MD.TRANSFLOW",
...
candedit =true;//rightInfo.candedit;  
cansave = true;//rightInfo.cansave;
function main()
{    
    Ext.QuickTips.init();
...
//For add customized properties end
	...
	var library = new LibraryPanel('selAnaly');
  ...
  graph.dblClick = function(evt, cell)
{
    ...
    var parentNode = library.getParentNode("数据模型");
    insertImageTemplate(library, graph, '业务模型', 'images/symbols/server.png', false,parentNode);
    insertImageTemplate(library, graph, '文本文件', 'images/symbols/file_txt.png', false,parentNode);
    insertImageTemplate(library, graph, 'Excel文件', 'images/symbols/file_excel.png', false,parentNode);
    var parentNode = library.getParentNode("数据查询");
    insertImageTemplate(library, graph, '新建查询', 'images/symbols/search.png', true,parentNode);
    insertImageTemplate(library, graph, '数据清单', 'images/symbols/column.png', true,parentNode);
    insertImageTemplate(library, graph, '清单下载', 'images/symbols/down.jpg', true,parentNode);
    insertImageTemplate(library, graph, '异步请求', 'images/symbols/error.png', true,parentNode);
    insertImageTemplate(library, graph, '作业计划', 'images/symbols/timer.png', true,parentNode);
    insertImageTemplate(library, graph, '数据抽样', 'images/symbols/link.png', true,parentNode);
    
    var parentNode = library.getParentNode("数据分析");
    insertImageTemplate(library, graph, '新建分析', 'images/house.png', false,parentNode);
    insertImageTemplate(library, graph, '分析报表', 'images/rep.gif', true,parentNode);
   
    var parentNode = library.getParentNode("跟踪评估");
    insertImageTemplate(library, graph, '新建跟踪', 'images/multiple.png', false,parentNode);
    insertImageTemplate(library, graph, '跟踪详细', 'images/column.png', false,parentNode);
    insertImageTemplate(library, graph, '演进分析', 'images/server.png', false,parentNode);
    insertImageTemplate(library, graph, '跟踪报表', 'images/workplace.png', false,parentNode);
   /* 
    ...
    var parentNode = library.getParentNode("数据模型");
    insertVertexTemplate(library, graph, '业务模型', 'images/swimlane.gif', null, 200, 200, 'Container',parentNode);
    insertVertexTemplate(library, graph, 'sql模型', 'images/rectangle.gif', null, 100, 40,'',parentNode);
    insertVertexTemplate(library, graph, '文本文件', 'images/rounded.gif', 'rounded=1', 100, 40,'',parentNode);
    insertVertexTemplate(library, graph, 'NOSQL', 'images/rounded.gif', 'rounded=1', 100, 40,'',parentNode);
    var parentNode = library.getParentNode("数据模型");
    insertVertexTemplate(library, graph, 'Ellipse', 'images/ellipse.gif', 'ellipse', 60, 60,'',parentNode);
    insertVertexTemplate(library, graph, 'Double Ellipse', 'images/doubleellipse.gif', 'ellipse;shape=doubleEllipse','', 60, 60,parentNode);
    insertVertexTemplate(library, graph, 'Triangle', 'images/triangle.gif', 'triangle', 40, 60,'',parentNode);
    insertVertexTemplate(library, graph, 'Rhombus', 'images/rhombus.gif', 'rhombus', 60, 60,'',parentNode);
	  insertVertexTemplate(library, graph, 'Horizontal Line', 'images/hline.gif', 'line', 120, 10,'',parentNode);
    insertVertexTemplate(library, graph, 'Hexagon', 'images/hexagon.gif', 'shape=hexagon', 80, 60,'',parentNode);
    insertVertexTemplate(library, graph, 'Cylinder', 'images/cylinder.gif', 'shape=cylinder', 60, 80,'',parentNode);
    insertVertexTemplate(library, graph, 'Actor', 'images/actor.gif', 'shape=actor', 40, 60,'',parentNode);
    insertVertexTemplate(library, graph, 'Cloud', 'images/cloud.gif', 'ellipse;shape=cloud', 80, 60,'',parentNode);
    */
   /*
...
function insertImageTemplate(panel, graph, name, icon, round,parentNode)
{
   /// var imagesNode = panel.images;
    var style = (round) ? 'roundImage' : 'image';
    return insertVertexTemplate(panel, graph, name, icon, style+';image='+icon, 50, 50, name, parentNode);
};
