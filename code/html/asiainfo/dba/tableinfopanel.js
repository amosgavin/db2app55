// 表格创建/信息显示面板 定义
Asiainfo.TableInfoPanel = function(config) {
	// 助手类
	var utils = new Asiainfo.TableInfoPanelUtils(this);
	...
	// 创建字段信息Panel
	var panel = new Asiainfo.TableColumnPanel({
	...
	// call 父类构建器
	Asiainfo.TableInfoPanel.superclass.constructor.call(this, config);
...
Ext.extend(Asiainfo.TableInfoPanel, Ext.Panel, {
	/**
	 * 设置面板中元素大小
	 */
	...
	createTable : function(callback) {
		...
		if (tableName) {
			...
			if (sql && sql.length > 0) {
				...
				Ext.Ajax.request({
					url : '/'+contextUrl+'/newrecordService'+'?command=executeSQL',
					scope : this,
					params : {
						sql : sql
					},
