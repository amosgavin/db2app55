// Table 列信息面板
Asiainfo.TableColumnPanel = function(config) {
	// create grid and form and link
	var grid = new Asiainfo.TableColumnGrid({
		isCreate : config.isCreate,
		readOnly : config.readOnly
	});
	grid.columnWidth = .7;
	var form = new Asiainfo.TableColumnExtendForm({
		isCreate : config.isCreate,
		readOnly : config.readOnly
	});
	form.columnWidth = .3;
	...
	// call
	Asiainfo.TableColumnPanel.superclass.constructor.call(this, config);
...
Ext.extend(Asiainfo.TableColumnPanel, Ext.Panel, {
	/*
	 * 调整控件大小
	 */
