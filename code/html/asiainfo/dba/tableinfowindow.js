// 表格创建/属性显示窗口 定义
Asiainfo.TableInfoWindow = function(config) {
	// 取得当前节点
	//var node = config.node;
	//var nodes = node.getPath('text').split("/");

	// 创建TableInfoPanel
	var tableInfoPanel = new Asiainfo.TableInfoPanel({
		isCreate : config.isCreate,// 是否是创建表（反之是查看表格属性）
		readOnly : config.readOnly,// 是否只读
		databaseName : 'db819',
		schemaName : 'md',
		tableName : config.isCreate ? 'NewTable' : 'test'
	});
	...
	var title = config.isCreate ? '创建表' : '查看属性 - ' + 'test';
	var cfg = {
	...
	// call 父类构建器
	Asiainfo.TableInfoWindow.superclass.constructor.call(this, config);
};
Ext.extend(Asiainfo.TableInfoWindow, Ext.Window, {
	/*
	 * 重新设置大小
	 */
	resetSize : function() {
