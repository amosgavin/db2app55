...
Ext.tree.ColumnTree.prototype.SetCurNodeField =function(fieldname,value){
	var colindex=-1;
 
	for(var i=0;i<this.columns.length;i++){
		if(this.columns[i].dataIndex==fieldname) {colindex=i;break;}
	}
	if(i!=-1){
	var selectedNode = this.getSelectionModel().getSelectedNode();
	if(selectedNode.ui.elNode.childNodes[i] && selectedNode.ui.elNode.childNodes[i].firstChild)
  selectedNode.ui.elNode.childNodes[i].firstChild.innerHTML = value;
  }
}
...
Ext.tree.ColumnNodeUI = Ext.extend(Ext.tree.TreeNodeUI, {
    ...
    renderElements : function(n, a, targetNode, bulkRender){
		...
		n.cols = new Array();
				...
				var text = n.text || (c.renderer ? c.renderer(a[c.dataIndex], n, a,c.dataIndex) : a[c.dataIndex]);
        ...
        var buf = [
             '<li class="x-tree-node" unselectable="on"><div ext:tree-node-id="',n.id,'" class="x-tree-node-el x-tree-node-leaf ', a.cls,'" unselectable="on">',
                '<div class="x-tree-col" style="width:',c.width-bw,'px;" unselectable="on">',
                    ...
                    '<span field="'+cols[0].dataIndex+'" unselectable="on">', text,"</span></a>",
                "</div>"];
         for(var i = 1, len = cols.length; i < len; i++){
             c = cols[i];
			 var text = (c.renderer ? c.renderer(a[c.dataIndex], n, a,c.dataIndex) : a[c.dataIndex]);	
			 n.cols[cols[i].dataIndex] = text;
             buf.push('<div class="x-tree-col ',(c.cls?c.cls:''),'" style="width:',c.width-bw,'px;" unselectable="on">',
                        '<div class="x-tree-col-text" field="'+c.dataIndex+'" unselectable="on">',text,"</div>",
                      "</div>");
         }
         buf.push(
...
Ext.extend(Ext.tree.ColumnTreeEditor, Ext.Editor, {
	...
	if(this.tree.ds && this.tree.ds.curRecord){
	if(this.editCol.field) this.tree.ds.curRecord.set(this.editCol.field,value);
	      dataManager.fresh(this.tree.ds,null,this.tree.ds.itemindex);		
    }},
