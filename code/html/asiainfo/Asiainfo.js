...
Asiainfo = function() {
	return {
		...
		addTabSheet: function(id,title,srcUrl){
			...
			if(!pg_index && !QoDesk){
				var newwin=window.open('../'+srcUrl,"","top=0, left=0, toolbar=yes, menubar=yes, scrollbars=yes, resizable=yes,location=yes, status=yes");newwin.resizeTo(window.screen.availWidth,window.screen.availHeight);
				return;
			}
			else if(pg_index){
		...
		rigisterProfile: function(paraname,paravalue){
		  ...
		  if(_UserInfo.profile[paraname])
		    var sql="update MD.METAUSER_PROFILE set PARAVALUE='"+paravalue+"' where USERNAME='"+_UserInfo.username+"' and PARANAME='"+paraname+"'"
		  else 
		  	var sql="insert into MD.METAUSER_PROFILE(USERNAME,PARANAME,PARAVALUE) values('"+_UserInfo.username+"','"+paraname+"','"+paravalue+"')"; 
    ...
    /**
		...
		readProfile: function(paraname){
		  if(!_UserInfo.profile){
		  	_UserInfo.profile = {};
		  	var tmpstore=  Asiainfo.getStore("select * from MD.METAUSER_PROFILE where USERNAME='"+_UserInfo.username+"'");
    ...
    /**
      ...
      if (xmlHttp.responseText) {
		 ...
		 */
			...
			sql :sql,
			initUrl : '/' + contextPath + '/pst/newrecordService', url : '/' + contextPath + '/pst/newrecordService',
		...
		executeSQL : function (sql,popTips,dataSource,callback){
			...
			var result = Ext.util.JSON.decode(Asiainfo.remoteData(url,popTips,callback));
		  return result;
		},
		...
		DataTransfer: function(sourceDB,targetDB,transferSql,targetTable,dataColumn){
			...
			return Ext.decode(Asiainfo.remoteData('/'+contextPath+'/pst/dataMigration?sourceDs='+sourceDB+'&targetDs='+targetDB+'&migInTabname='+targetTable+'&migInColumns='+dataColumn+'&migDataSql='+transferSql));
		},
		 /**
		 * 数据迁移管理，从一个数据库导数据到另外一个数据库，应用于小数据量的实时调用数据迁移
		 * @param sourceDB    源数据源名
      ...
      return  Asiainfo.remoteData('/'+contextPath+'/pst/dataMigration?migDataSql='+transferSql+'&migInTabname='+targetTable+'&isTrancateSourceTable='+targetFlag+'&sourceDs='+sourceDB+'&targetDs='+targetDB+'&migInColumns='+importColumn);
 
		},
		saveChangeLog: function(logtype,reobjid,opname,remark,opstroe,content){
var ds_log = new Asiainfo.data.AsiaInfoJsonStore({
	     sql:"select LOGTYPE, REFOBJID, OPTIME, OPNAME, OPTYPE, EXCUTER, CONTENT, REMARK,RESULT from MD.OP_LOG where 1>2",
...
if(opstroe.getModifiedRecords().length>0){
  ...
  //alert('kkk');
		...
		RightSet:function(){
		  ...
		  baseFun.loadScript('../asiainfo/form/RightSet.js');
		  ShowRightSet();
		},
		...
		GetNewCode:function(store,fieldname,pre,tabname){
		  if(store){///根据store取的新编码
		   ...
		   return pre+seq 	
                   ...
                   var _tmpsql="select max(int(substr("+fieldname+","+(pre.length+5)+",3))) as seq from "+tabname+" where "+fieldname+" like '%"+pre+cycle+"%'"
...
baseFun.loadScript('../ext/resources/css/xtheme-gray.css','css'); 
function CheckObjExists(objtype,objname){
	var _tmpsql;
	if(objtype=='流程' || objtype=='数据流程') _tmpsql="select flowcode  from md.transflow where ucase(flowcode)='"+objname.toUpperCase()+"'"
	else if(objtype=='表') _tmpsql="SELECT  1 FROM MD.TABLEFILE where ucase(DATANAME)='"+objname.toUpperCase()+"'"
	else if(objtype=='程序') _tmpsql="SELECT  1 FROM MD.PROC where ucase(PROC_NAME)='"+objname.toUpperCase()+"'"
	else if(objtype=='应用') _tmpsql="select  1 from md.portal  where ucase(portalcode)='"+objname.toUpperCase()+"'"
	else if(objtype=='主题') _tmpsql="select  1 from md.topic  where ucase(TOPICCODE)='"+objname.toUpperCase()+"'"
	else return false;
