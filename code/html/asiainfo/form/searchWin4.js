...
//		});
		...
		*/
		...
		var searchBtn_handler = function(){
			...
			s = tsql+" where UCASE(VALUES1) LIKE '%"+v+"%' or UCASE(VALUES2) LIKE '%"+v+"%' order by VALUES1";
			 
			_store.baseParams.initSql = s;
	 
			_store.load({params:{initSql:s,start:0,limit:12}});
		};
	  ...
	  //		_this.confirmBtn.disable();
		...
		this.e_func = function(trigger_e,sql_second,db,storeId){
			...
			_this.store.load( {params:{start:0,limit:12}} );
