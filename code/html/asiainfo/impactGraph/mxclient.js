// You are licensed to use this software under the terms of the JGraph Evaluation License. Please ask for a copy if you require one.
var mxClient=
{
VERSION:'0.99.0.0',
...
if(c!=null&&pt!=null&&(false||false)&&(c.scrollLeft>0||c.scrollTop>0))
{
...
makeDraggable:function(element,graph,funct,dragElement,dx,dy,autoscroll,scalePreview)
{
dx=(dx!=null)?dx:0;
...
{
function mxMouseEvent(source,evt,state,tooltip)
{
this.source=source;
this.evt=evt;
this.state=state;
...
mxMouseEvent.prototype.source=null;
...
mxEventSource.prototype.fireEvent=function(name)
{
...
redirectMouseEvents:function(element,graph,cell,index,transparent,doubleClick,forceTransparent)
{
...
{
graph.dblClick(evt,checkCell(evt));
mxEvent.consume(evt);
});
}
},
...
{
return evt.button==2||mxEvent.isShiftDown(evt);
},
...
this.fireEvent(mxEvent.ACTIVATE,this,new mxEventObject([oldWindow]));
}
};
...
self.updateShadow();
self.fireEvent(mxEvent.RESIZE,self,evt);
mxEvent.consume(evt);
};
var dropHandler=function(evt)
{
mxEvent.removeListener(document,'mousemove',dragHandler);
mxEvent.removeListener(document,'mouseup',dropHandler);
self.fireEvent(mxEvent.RESIZE_END,self,evt);
mxEvent.consume(evt);
};
mxEvent.addListener(document,'mousemove',dragHandler);
mxEvent.addListener(document,'mouseup',dropHandler);
self.fireEvent(mxEvent.RESIZE_START,self,evt);
mxEvent.consume(evt);
});
...
self.updateShadow();
self.fireEvent(mxEvent.MINIMIZE,self,evt);
}
else
{
minimized=false;
...
self.updateShadow();
self.fireEvent(mxEvent.NORMALIZE,self,evt);
}
mxEvent.consume(evt);
};
...
self.fireEvent(mxEvent.MAXIMIZE,self,evt);
}
else
{
maximized=false;
...
self.fireEvent(mxEvent.MOVE,self,evt);
mxEvent.consume(evt);
};
var dropHandler=function(evt)
{
mxEvent.removeListener(document,'mousemove',dragHandler);
mxEvent.removeListener(document,'mouseup',dropHandler);
self.fireEvent(mxEvent.MOVE_END,self,evt);
mxEvent.consume(evt);
};
mxEvent.addListener(document,'mousemove',dragHandler);
mxEvent.addListener(document,'mouseup',dropHandler);
self.fireEvent(mxEvent.MOVE_START,self,evt);
mxEvent.consume(evt);
});
};
...
{
self.fireEvent(mxEvent.CLOSE,self,evt);
...
this.fireEvent(mxEvent.SHOW,this);
};
mxWindow.prototype.hide=function()
{
this.div.style.display='none';
this.fireEvent(mxEvent.HIDE,this);
};
mxWindow.prototype.destroy=function()
{
this.fireEvent(mxEvent.DESTROY,this);
if(this.div!=null)
{
...
self.fireEvent(mxEvent.SELECT,self,new mxEventObject([null]));
funct();
});
this.container.appendChild(img);
...
this.fireEvent(mxEvent.SELECT,this,new mxEventObject([funct]));
}
};
...
{
this.connected=true;
this.fireEvent(mxEvent.CONNECT,this);
}
...
{
self.connected=true;
self.fireEvent(mxEvent.CONNECT,self);
self.poll();
});
}
};
...
{
this.suspended=true;
this.fireEvent(mxEvent.SUSPEND,this);
}
};
...
this.fireEvent(mxEvent.RESUME,this);
if(!this.polling)
{
this.poll();
}
...
this.fireEvent(mxEvent.DISCONNECT,this,new mxEventObject([reason]));
};
...
this.fireEvent(mxEvent.NOTIFY,this,new mxEventObject([this.urlNotify,xml]));
}
};
...
self.fireEvent(mxEvent.GET,self,new mxEventObject([url,req]));
...
this.model.fireEvent(mxEvent.UNDO,this.model,new mxEventObject([edit]));
this.model.fireEvent(mxEvent.CHANGE,this.model,new mxEventObject([changes]));
this.fireEvent(mxEvent.FIRED,this,new mxEventObject([changes]));
}
}
this.fireEvent(mxEvent.RECEIVE,this,new mxEventObject([node]));
}
};
...
{
edit.source.fireEvent(mxEvent.CHANGE,edit.source,new mxEventObject([edit.changes]));
edit.source.fireEvent(mxEvent.NOTIFY,edit.source,new mxEventObject([edit.changes]));
}
return edit;
};
...
{
this.fireEvent(mxEvent.UNDO,this,new mxEventObject([edit]));
break;
}
}
};
...
if(edit.isSignificant())
{
this.fireEvent(mxEvent.REDO,this,new mxEventObject([edit]));
break;
}
}
};
...
this.fireEvent(mxEvent.ADD,this,new mxEventObject([undoableEdit]));
};
...
{
return mxEvent.isPopupTrigger(evt)||(this.useLeftButtonForPopup&&mxEvent.isLeftMouseButton(evt));
};
...
{
this.graph.setCellStyles(mxConstants.STYLE_NOEDGESTYLE,(value)?'1':'0',[edge]);
};
...
{
var result=new Array();
this.filterDescendants(null,parent,result);
return result;
};
...
this.filterDescendants(filter,child,result);
}
return result;
};
...
{
this.fireEvent(mxEvent.BEFORE_EXECUTE,this,new mxEventObject([change]));
change.execute();
this.fireEvent(mxEvent.EXECUTE,this,new mxEventObject([change]));
this.beginUpdate();
this.currentEdit.add(change);
this.endUpdate();
this.fireEvent(mxEvent.AFTER_EXECUTE,this,new mxEventObject([change]));
};
mxGraphModel.prototype.beginUpdate=function()
{
this.updateLevel++;
this.fireEvent(mxEvent.BEGIN_UPDATE,this);
};
...
this.fireEvent(mxEvent.END_UPDATE,this,new mxEventObject([this.currentEdit]));
try
{
if(this.endingUpdate&&!this.currentEdit.isEmpty())
{
this.fireEvent(mxEvent.BEFORE_UNDO,this,new mxEventObject([this.currentEdit]));
var tmp=this.currentEdit;
this.currentEdit=this.createUndoableEdit();
tmp.notify();
this.fireEvent(mxEvent.UNDO,this,new mxEventObject([tmp]));
}
}
finally
{
...
this.selectionModel.fireEvent(mxEvent.CHANGE,this,new mxEventObject([this.removed,this.added]));
};
}
...
{
overlay.fireEvent(mxEvent.CLICK,overlay,new mxEventObject([evt,state.cell]));
});
...
var bounds=(isEdge)?new mxRectangle(state.x+state.width/2-w/2*s,state.y+state.height/2-h/2*s,w*s,h*s)
...
this.fireEvent(mxEvent.SCALE_AND_TRANSLATE,this,new mxEventObject([oldScale,scale,oldDx,oldDy,dx,dy]));
};
...
this.fireEvent(mxEvent.SCALE,this,new mxEventObject([oldScale,scale]));
};
...
this.fireEvent(mxEvent.TRANSLATE,this,new mxEventObject([oldDx,oldDy,dx,dy]));
};
...
this.view.fireEvent(name,this.view,new mxEventObject([this.previous,this.view.currentRoot]));
if(this.isUp)
{
...
mxGraph.prototype.cellsSizable=true;
...
this.clearSelection();
this.fireEvent(mxEvent.ROOT,this);
}
...
this.fireEvent(mxEvent.ADD_OVERLAY,this,new mxEventObject([cell,overlay]));
return overlay;
};
...
this.fireEvent(mxEvent.REMOVE_OVERLAY,this,new mxEventObject([cell,overlay]));
}
else
{
overlay=null;
...
for(var i=0;i<overlays.length;i++)
{
this.fireEvent(mxEvent.REMOVE_OVERLAY,this,new mxEventObject([cell,overlays[i]]));
}
}
return overlays;
};
...
if(cell!=null)
{
this.fireEvent(mxEvent.START_EDITING,this,new mxEventObject([cell,trigger]));
...
this.fireEvent(mxEvent.LABEL_CHANGED,this,new mxEventObject([cell,newValue,trigger]));
}
finally
{
...
{
this.fireEvent(mxEvent.CLICK,this,new mxEventObject([evt,cell]));
...
{
this.fireEvent(mxEvent.DOUBLE_CLICK,this,new mxEventObject([evt,cell]));
...
this.fireEvent(mxEvent.SIZE,this,new mxEventObject([bounds]));
};
...
this.resetEdge(edge);
this.fireEvent(mxEvent.FLIP_EDGE,this,new mxEventObject([edge]));
}
finally
{
this.model.endUpdate();
...
this.fireEvent(mxEvent.ORDER_CELLS,this,new mxEventObject([back,cells]));
}
finally
{
this.model.endUpdate();
...
this.cellsMoved(cells,-bounds.x,-bounds.y,false);
index=this.model.getChildCount(parent);
this.cellsAdded([group],parent,index,null,null,false);
this.cellsResized([group],[bounds]);
this.fireEvent(mxEvent.GROUP_CELLS,this,new mxEventObject([group,border,cells]));
}
finally
{
this.model.endUpdate();
...
this.fireEvent(mxEvent.REMOVE_CELLS_FROM_PARENT,this,new mxEventObject([cells]));
}
finally
{
this.model.endUpdate();
...
this.fireEvent(mxEvent.ADD_CELLS,this,new mxEventObject([cells,parent,index,source,target]));
}
finally
{
this.model.endUpdate();
...
this.fireEvent(mxEvent.REMOVE_CELLS,this,new mxEventObject([cells,includeEdges]));
}
finally
{
this.model.endUpdate();
...
this.cellsMoved(cells,dx,dy,false);
...
this.fireEvent(mxEvent.TOGGLE_CELLS,this,new mxEventObject([show,cells,includeEdges]));
}
finally
{
this.model.endUpdate();
...
this.fireEvent(mxEvent.FOLD_CELLS,this,new mxEventObject([collapse,recurse,cells]));
}
finally
{
this.model.endUpdate();
...
this.fireEvent(mxEvent.UPDATE_CELL_SIZE,this,new mxEventObject([cell,ignoreChildren]));
}
finally
{
this.model.endUpdate();
...
{
cellStyle=mxUtils.setStyle(cellStyle,mxConstants.STYLE_STARTSIZE,size.width);
if(collapsed)
{
...
{
cellStyle=mxUtils.setStyle(cellStyle,mxConstants.STYLE_STARTSIZE,String.valueOf(size.getHeight()));
if(collapsed)
{
geo.height=size.height;
}
...
this.fireEvent(mxEvent.RESIZE_CELLS,this,new mxEventObject([cells,boundsArray]));
}
finally
{
this.model.endUpdate();
...
this.fireEvent(mxEvent.MOVE_CELLS,this,new mxEventObject([cells,dx,dy,clone,target,evt]));
}
return cells;
};
...
this.view.validate();
this.sizeDidChange();
this.fireEvent(mxEvent.REFRESH,this);
};
...
mxGraph.prototype.isCellSizable=function(cell)
{
return this.isCellsSizable()&&!this.isCellLocked(cell);
};
mxGraph.prototype.isCellsSizable=function()
{
return this.cellsSizable;
};
mxGraph.prototype.setCellsSizable=function(value)
{
this.cellsSizable=value;
};
...
if(!false&&evtName=='mousedown'&&this.activeElement!=null)
{
...
if(!this.isEditing()&&(false||false||false||evt.target!=this.container))
{
...
if(false&&document.selection.type!='None'&&evtName!='mousedown')
{
try
{
...
this.fireEvent(mxEvent.LAYOUT_CELLS,this,new mxEventObject([cells]));
}
finally
{
model.endUpdate();
...
{
function mxResizeManager(graph,shiftRightwards,shiftDownwards,extendParents)
{
var self=this;
...
mxResizeManager.prototype=new mxEventSource();
mxResizeManager.prototype.constructor=mxResizeManager;
mxResizeManager.prototype.graph=null;
mxResizeManager.prototype.enabled=true;
mxResizeManager.prototype.shiftRightwards=true;
mxResizeManager.prototype.shiftDownwards=true;
mxResizeManager.prototype.extendParents=true;
mxResizeManager.prototype.resizeHandler=null;
mxResizeManager.prototype.foldHandler=null;
mxResizeManager.prototype.isCellIgnored=function(cell)
{
return!this.getGraph().getModel().isVertex(cell);
};
mxResizeManager.prototype.isCellShiftable=function(cell)
{
return this.getGraph().getModel().isVertex(cell)&&this.getGraph().isCellMovable(cell);
};
mxResizeManager.prototype.isEnabled=function()
{
return this.enabled;
};
mxResizeManager.prototype.setEnabled=function(value)
{
this.enabled=value;
};
mxResizeManager.prototype.isShiftRightwards=function()
{
return this.shiftRightwards;
};
mxResizeManager.prototype.setShiftRightwards=function(value)
{
this.shiftRightwards=value;
};
mxResizeManager.prototype.isShiftDownwards=function()
{
return this.shiftDownwards;
};
mxResizeManager.prototype.setShiftDownwards=function(value)
{
this.shiftDownwards=value;
};
mxResizeManager.prototype.isExtendParents=function()
{
return this.extendParents;
};
mxResizeManager.prototype.setExtendParents=function(value)
{
this.extendParents=value;
};
mxResizeManager.prototype.getGraph=function()
{
return this.graph;
};
mxResizeManager.prototype.setGraph=function(graph)
{
if(this.graph!=null)
{
...
mxResizeManager.prototype.cellsResized=function(cells)
{
if(cells!=null)
{
var model=this.graph.getModel();
...
mxResizeManager.prototype.cellResized=function(cell)
{
...
mxResizeManager.prototype.shiftCell=function(cell,dx,dy,Ox0,y0,right,bottom,fx,fy,extendParent)
{
var graph=this.getGraph();
...
mxResizeManager.prototype.getCellsToShift=function(state)
{
var graph=this.getGraph();
...
mxResizeManager.prototype.destroy=function()
{
this.setGraph(null);
};
}
...
{
if(this.isEnabled()&&this.graph.isEnabled()&&!this.graph.isForceMarqueeEvent(evt)&&index==null&&cell!=null)
{
this.cell=null;
...
var model=this.graph.model;
var geo=model.getGeometry(cell);
if(this.graph.isCellMovable(cell)&&((!model.isEdge(cell)||this.graph.getSelectionCount()>1||(geo.points!=null&&geo.points.length>0)||model.getTerminal(cell,true)==null||model.getTerminal(cell,false)==null)||this.graph.allowDanglingEdges||(this.graph.isCloneEvent(evt)&&this.graph.isCellsCloneable())))
{
...
this.highlight.init(this.shape.node.parentNode);
if(this.shape.dialect==mxConstants.DIALECT_SVG)
{
...
var scale=graph.getView().scale;
if(graph.isGridEnabledEvent(evt))
{
...
if(graph.isConstrainedEvent(evt))
{
if(Math.abs(dx)>Math.abs(dy))
{
dy=0;
...
var clone=graph.isCloneEvent(evt)&&graph.isCellsCloneable()&&this.cloneEnabled;
...
if(this.highlight.dialect==mxConstants.DIALECT_STRICTHTML)
{
bounds.x-=2;
bounds.y-=2;
}
...
if(this.graph.isSplitEnabled()&&this.graph.isSplitTarget(target,cells))
{
graph.splitEdge(target,cells,null,dx,dy);
}
else
{
this.moveCells(this.graph.getSelectionCells(),dx,dy,clone,this.target,evt);
}
}
}
...
{
return(this.useLeftButtonForPanning&&(this.ignoreCell||(cell==null))&&mxEvent.isLeftMouseButton(evt))||(this.useShiftKey&&mxEvent.isShiftDown(evt))||(this.usePopupTrigger&&mxEvent.isPopupTrigger(evt));
};
...
this.shape.dialect=mxConstants.DIALECT_STRICTHTML;
this.shape.init(graph.container);
}
else
{
...
{
shape=this.shape;
var bounds=new mxRectangle(this.markedState.x,this.markedState.y,this.markedState.width,this.markedState.height);
bounds.x-=2;
bounds.y-=2;
if(shape.dialect!=mxConstants.DIALECT_STRICTHTML)
{
bounds.width+=4;
bounds.height+=4;
}
...
this.fireEvent(mxEvent.MARK,this,new mxEventObject([this.markedState]));
}
};
...
this.edgeShape.node.style.display='none';
this.shape.node.style.display='none';
this.fireEvent(mxEvent.MARK,this,new mxEventObject([null]));
};
...
{
self.icon=icon;
self.graph.dispatchGraphEvent('mousedown',evt,(self.currentState!=null)?self.currentState.cell:state.cell);
});
mxEvent.addListener(icon.node,'mousemove',function(evt)
{
self.graph.dispatchGraphEvent('mousemove',evt,(self.currentState!=null)?self.currentState.cell:state.cell);
});
mxEvent.addListener(icon.node,'mouseup',function(evt)
{
self.graph.dispatchGraphEvent('mouseup',evt,(self.currentState!=null)?self.currentState.cell:state.cell);
});
icons.push(icon);
...
{
if(this.isEnabled()&&this.graph.isEnabled()&&!this.graph.isForceMarqueeEvent(evt)&&this.previous!=null&&this.error==null&&((this.icons==null)||(this.icons!=null&&this.icon!=null))&&index==null)
{
...
if(this.graph.isCellSizable(this.state.cell))
{
var i=0;
if(!this.singleSizer)
{
...
else if(this.graph.isCellMovable(this.state.cell)&&!this.graph.isCellSizable(this.state.cell)&&this.state.width<2&&this.state.height<2)
{
...
{
if(this.graph.isGridEnabledEvent(evt))
{
point.x=this.graph.snap(point.x/scale)*scale;
point.y=this.graph.snap(point.y/scale)*scale;
}
...
{
var edge=this.connect(edge,this.marker.getValidState().cell,this.isSource,this.graph.isCloneEvent(evt)&&this.cloneEnabled&&this.graph.isCellsCloneable());
}
...
editor.fireEvent(mxEvent.ESCAPE,editor,evt);
};
}
};
...
{
editor.graph.cellsToBack();
});
this.addAction('toFront',function(editor,cell)
{
editor.graph.cellsToFront();
});
...
{
self.fireEvent(mxEvent.SESSION,self,new mxEventObject([session]));
};
session=this.connect(this.urlInit,this.urlPoll,this.urlNotify,sessionChanged);
session.addListener(mxEvent.DISPATCHED,function(sender,evt)
{
var changes=evt.getArgAt(0);
...
{
graph.fireEvent(mxEvent.DOUBLE_CLICK,graph,new mxEventObject([evt,cell]));
...
{
self.fireEvent(mxEvent.ROOT,self);
};
...
if(filename!=null)
{
this.fireEvent(mxEvent.BEFORE_OPEN,this,new mxEventObject([filename]));
var e=null;
try
{
...
this.fireEvent(mxEvent.OPEN,this,new mxEventObject([filename]));
}
catch(ex)
{
...
this.fireEvent(mxEvent.AFTER_OPEN,this,new mxEventObject([filename,e]));
}
};
...
{
this.fireEvent(mxEvent.BEFORE_SAVE,this,new mxEventObject([isAutomatic]));
this.lastSnapshot=new Date().getTime();
this.ignoredChanges=0;
this.modified=false;
var e=null;
...
this.fireEvent(mxEvent.SAVE,this,new mxEventObject([isAutomatic,url]));
}
catch(ex)
{
e=ex;
}
this.fireEvent(mxEvent.AFTER_SAVE,this,new mxEventObject([isAutomatic,e]));
}
};
...
{
self.fireEvent(mxEvent.POST,self,new mxEventObject([req,url,data]));
});
};
...
this.fireEvent(mxEvent.BEFORE_ADD_VERTEX,this,new mxEventObject([vertex,parent]));
model.beginUpdate();
try
{
...
this.fireEvent(mxEvent.ADD_VERTEX,this,new mxEventObject([vertex]));
}
}
finally
{
...
this.fireEvent(mxEvent.AFTER_ADD_VERTEX,this,new mxEventObject([vertex]));
}
return vertex;
};
