	...
	*/
	this.cmpFresh = function(cmp,store,cmptype,orignCmp,orignCmptype){
		if(cmptype=='Field'){
			...
			if(store && store.curRecord && store.curRecord.data && store.curRecord.data[cmp.name] && cmp && cmp.setValue)
...
Asiainfo.RightMgr = function(){
  ...
  this.ds_right = new Asiainfo.data.AsiaInfoJsonStore({
	     sql:"select MODELCODE, PARANAME,CNNAME,TYPE,DEFAULT,VAL,REMARK from MD.METAMODELCFG where modelcode='"+modelcode+"' and (VAL is not null or REMARK is not null)",
	     initUrl:'/'+contextPath+'/pst/newrecordService',
	     url:'/'+contextPath+'/pst/newrecordService',
	     root:'root',
	     table:'MD.METAMODELCFG',
