  ...
  try {
		...
		result = Ext.util.JSON.decode(conn.responseText);
		 if (result.success == true){
			if(showTips!=false) Ext.Msg.alert("提示[syncRequest]", result.msg);
		}
		 else 
				  alert("错误, "+result.msg);
		 } catch (e) {  alert("Exception,远程操作出错:"); };
   } else { Ext.Msg.alert("Exception", "远程无结果返回:"); }
...
Ext.extend(Asiainfo.data.AsiaInfoJsonStore,Ext.data.JsonStore,{
 	 JsonStoreInit:function(){
   	  ...
   	  initurl = Asiainfo.URLEncode(initurl);
			...
			if(conn.responseText=='null' ){alert('执行sql出错:'+c.sql);return };
	...
	if(!options)
	  ...
	  var qrySql=this.sql;
		 ...
		 if(this.sortInfo && this.sortInfo.field) qrySql ="select * from ("+qrySql+" ) as QryTab order by "+this.sortInfo.field +" "+this.sortInfo.direction;
		 	
		if((options.params)){
			if(options.params.limit > 0){
				options.params = {command:'query',
						  initSql:qrySql,
	...
	sendAjax:function(_r){
		...
		var requestObject = Ext.Ajax.request({
			...
			failure:_r.failure || function(o,r){Ext.Msg.alert("错误", r.responseText);},
			params:_r.params
		});
