...
<body>
    ...
    <ai:contractframe title="comframe.html.workflow.WorkflowInst42" id="comframe.html.workflow.WorkflowInst42" i18nRes="i18n.comframe_resource" width="100%">
  	  <div class="t-bot-mc-button">
      	<ai:button id="resumesBtn" i18nRes="i18n.comframe_resource" text="comframe.html.workflow.Alarm29" onclick="resumeExceptionWorkflows()"/>
      </div>
  		<ai:contractitem>
           ...
           <ai:table tableid="workflowTable" setname="com.ai.comframe.config.set.SETVmWF"
                    ...
                    implservice_querymethod="queryWorkflow(String regionID,String queueID,String busiOrderId,String busiOrderType,int state,String startTime,String endTime,int $STARTROWINDEX,int $ENDROWINDEX)"
                    implservice_countmethod="getVmWorkflowCount(String regionID,String queueID,String busiOrderId,String busiOrderType,int state,String startTime,String endTime)"
					width="100%" oncontextmenu="showWorkFlow"
					height="160" pagesize="7" onrowchange="showSrvErrMsg"
					needrefresh="true" multiselect="true" initial="false"
					editable="false" >
...
<ai:contractframe title="comframe.html.workflow.WorkflowInst84"  i18nRes="i18n.comframe_resource" id="comframe.html.workflow.WorkflowInst84" width="100%">
	...
	<ai:table tableid="task" setname="com.ai.comframe.config.set.SETVmTask"
			tablemodel="com.ai.appframe2.web.datamodel.MethodModelForService"
            implservice_name="com.ai.comframe.config.service.interfaces.IWorkflowConsoleSV"
            implservice_querymethod="queryVmTask(String workflowId,int workflowState)"
...
<ai:contractframe title="comframe.html.workflow.WorkflowInst118" i18nRes="i18n.comframe_resource" id="comframe.html.workflow.WorkflowInst118" width="100%">
	...
	<ai:table tableid="tasktrans" setname="com.ai.comframe.config.set.SETVmTaskTS"
			tablemodel="com.ai.appframe2.web.datamodel.MethodModelForService"
            implservice_name="com.ai.comframe.config.service.interfaces.IWorkflowConsoleSV"
            implservice_querymethod="queryVmTaskTrans(String parentTaskId,String workflowId)"
...
function resumeExceptionWorkflows(){	
	var rows = workflowTable.getSelectedRows();
	if(rows.length<1){
		 alert(g_I18NMessage("comframe_resources","comframe_html_workflow_WorkflowInst454"));	
	      return;
	}
	if(!confirm(g_I18NMessage("comframe_resources","comframe_html_workflow_WorkflowInst784"))){  
          return;
    }
	var workflowIds="";
    for(var i=rows.length;i>0;i--){ 
	    var workflowID = workflowTable.getValue(rows[i-1],"WORKFLOW_ID");
	    workflowIds=workflowIds+workflowID+",";
    }
    workflowIds=workflowIds.substring(0,workflowIds.length-1);
	var rtn = saveRowSet('<%=request.getContextPath()%>/business/com.ai.comframe.config.action.WorkflowAction?action=resumeExceptionWorkflows&workflowIds='+workflowIds+getCenterStr(regionID),false);
...
//双击工作流列表中某一条记录时，在task中显示相应的信息
	function query(){
		...
		var condition="workflowId="+workflowID+"&workflowState="+workflowState+getCenterStr(regionID);
...
function qryTaskTrans(){
  ...
  tasktrans.refresh("parentTaskId="+taskId+"&workflowId="+workflowId+getCenterStr(regionID));
}
...
function qryVmWF(){
		...
		if(queueID==-1||queueID==""){
 		...
 		var cond = "regionID="+document.all.regionID.value+"&queueID="+queueID;
...
var popMenuWorkFlow =null;
		...
		function buildPopMenuWorkFlow(state){
		 // alert(state);
		  ...
		  var msg = PostInfo(_gModuleName+"/business/com.ai.comframe.config.action.WorkflowAction?action=getWorkflowPopMenu&"+param,"");
...
function toSvg(){
	...
	if (index<0){
 	...
 	window.open("SvgView.jsp?workflow_id="+workflow_id+getCenterStr(regionID)+"&regionID="+regionID,"","scroll=yes,resizable=no,status=no,help=no,height=680px,width=720px");
...
function toDojo(){
	...
	if (index<0){
 	...
 	window.open("DojoView.jsp?workflow_id="+workflow_id+getCenterStr(regionID)+"&regionID="+regionID,"","scroll=yes,resizable=no,status=no,help=no,height=680px,width=1024px");
...
function showTask(){
    ...
    var lockStaff=g_TableRowSetManager.get("task").getValue(index,"LOCK_STAFF_ID");
     buildPopMenuTask(workflowstate,taskstate,lockStaff);
    if(popMenu != null){
      popMenu.showPopMenu();
    }
}
function buildPopMenuTask(workflowstate,state,lockStaff){
...
var strJosn = eval(msg.getValueByName("MESSAGE")); 
      ...
      for(var i=0; i<strJosn.length; i++){
          var isEnable=strJosn[i].enable;
          var func=strJosn[i].func;
          if(func=='lockTask' && lockStaff!=null && lockStaff!=""){
        	  isEnable='false';
          }      
          if(func=='releaseTask' && (lockStaff==null || lockStaff=="")){
        	  isEnable='false';
          }
         if(isEnable =='false'){             
...
function jumpToTask(){
	...
	var param = window.showModalDialog("GoBackTask.jsp?workflowId="+ workflowId+"&templateTag="+templateTag+"&regionID="+regionID+"&dealType=jumpToNode",null,"resizable:no;status:no;help:no;dialogHeight:750px;dialogWidth:630px");
	if(param == null){
	  return;
	}
...
function goBackToTask(){
	...
	var param = window.showModalDialog("GoBackTask.jsp?workflowId="+ workflowId+"&templateTag="+templateTag+"&regionID="+regionID+"&dealType=goBackToNode",null,"resizable:no;status:no;help:no;dialogHeight:750px;dialogWidth:630px");
	if(param == null){
	  return;
	}
...
function showTaskTrans(){
    ...
    var lockStaff=g_TableRowSetManager.get("tasktrans").getValue(index,"LOCK_STAFF_ID");   
   
    
     buildPopMenuTaskTrans(workflowstate,taskstate,lockStaff)
            if(popMenuTrans != null){
        ...
        function buildPopMenuTaskTrans(workflowstate,state,lockStaff){
		  ...
		  var strJosn = eval(msg.getValueByName("MESSAGE"));
                    ...
                    for(var i=0; i<strJosn.length; i++){
                        var isEnable=strJosn[i].enable;
                        var func=strJosn[i].func;
                        if(func=='lockTaskTrans' && lockStaff!=null && lockStaff!=""){
                      	  isEnable='false';
                        }      
                        if(func=='releaseTaskTrans' && (lockStaff==null || lockStaff=="")){
                      	  isEnable='false';
                        }
                       if(isEnable =='false'){             
