...
function OrgShow(OrgNode_){
	...
	this.Run=function(){
		...
		for(var n=1; n<=this.Depth; n++){//设置顶距离 
			var tempTop=this.Left + GetDepthHeightToRoot(n);//hxg  var tempTop=this.Top + GetDepthHeightToRoot(n);
			
			var tempNodes=this.DepthGroup[n].Nodes;
			for(var m=0; m<tempNodes.length; m++){ 
				tempNodes[m].Left=tempTop; //hxg tempNodes[m].Top=tempTop;
			}
		}
		...
		for(var n=this.Depth; n>=1; n--){//设置左距离
			...
			if(n==this.Depth){
				for(var m=0; m<DepthNodes.length; m++){
					if(m==0){
						DepthNodes[m].Top=0;//hxg DepthNodes[m].Left=0;
					}else{
						DepthNodes[m].Top=DepthNodes[m-1].Top + DepthNodes[m-1].Height + this.IntervalWidth;//hxg DepthNodes[m].Left=DepthNodes[m-1].Left + DepthNodes[m-1].Width + this.IntervalWidth;
					}
				}
			}else{
				for(var m=0; m<DepthNodes.length; m++){//根据下级节点位置，确定节点位置
					if(DepthNodes[m].Nodes.length!=0){
						var tempNodeLeft_=DepthNodes[m].Nodes[0].Top + (GetGroupWidthByNode(DepthNodes[m].Nodes[0]) / 2);//hxg var tempNodeLeft_=DepthNodes[m].Nodes[0].Left + (GetGroupWidthByNode(DepthNodes[m].Nodes[0]) / 2);
						tempNodeLeft_-=(DepthNodes[m].Height / 2);//tempNodeLeft_-=(DepthNodes[m].Width / 2);
						DepthNodes[m].Top = tempNodeLeft_;//DepthNodes[m].Left = tempNodeLeft_;
					}
				}
				for(var m=0; m<DepthNodes.length; m++){
					if(DepthNodes[m].Top==null){//没有下级节点的， hxg if(DepthNodes[m].Left==null){
						SetLeftByDepthNode(DepthNodes, m, "LTR");
					}
				}
				
				for(var m=1; m<DepthNodes.length; m++){//修正错误位置
					var ErrDistance=this.IntervalWidth - (DepthNodes[m].Top - DepthNodes[m-1].Top - DepthNodes[m-1].Height);//hxg var ErrDistance=this.IntervalWidth - (DepthNodes[m].Left - DepthNodes[m-1].Left - DepthNodes[m-1].Width);
					if(ErrDistance>0){
		...
		var MaxLeftValue=this.Nodes[0].Top;  //hxg var MaxLeftValue=this.Nodes[0].Left;
		for(var n=1; n<this.Nodes.length; n++){//取得最小左距离
			if(MaxLeftValue>this.Nodes[n].Top){//if(MaxLeftValue>this.Nodes[n].Left){
				MaxLeftValue=this.Nodes[n].Top;//MaxLeftValue=this.Nodes[n].Left;
			}
		}
		MaxLeftValue=(-MaxLeftValue) + this.Top;//MaxLeftValue=(-MaxLeftValue) + this.Left;
		for(var n=0; n<this.Nodes.length; n++){//重新设置距离
			this.Nodes[n].Top+=MaxLeftValue;//this.Nodes[n].Left+=MaxLeftValue;
			this.Nodes[n].Box.style.left=this.Nodes[n].Left + "px" //hxg ?
			this.Nodes[n].Box.style.top=this.Nodes[n].Top + "px"   //hxg ?
		}
			...
			for(var m=0; m<tempNodes.length; m++){
				if(n!=this.Depth){//设置底线条
					if(tempNodes[m].Nodes.length!=0){
						var tempLineLeft=tempNodes[m].Top + (tempNodes[m].Height / 2);//hxg var tempLineLeft=tempNodes[m].Left + (tempNodes[m].Width / 2);
						var tempLineHeight=((this.IntervalHeight - this.LineSize) / 2);
						tempLineHeight+=(this.DepthGroup[n].Width - tempNodes[m].Width);//hxg tempLineHeight+=(this.DepthGroup[n].Height - tempNodes[m].Height);
						var tempLineTop=tempNodes[m].Left + tempNodes[m].Width;//hxg var tempLineTop=tempNodes[m].Top + tempNodes[m].Height;
						...
						var tempBottomLine=new CreateLine(2, tempLineLeft ,tempLineTop,  this.LineSize, tempLineHeight,  this.LineColor, "LineBottom_"+ tempNodes[m].Id);
						tempNodes[m].BottomLine=tempBottomLine.Line;
					}
				}
				if(n!=1){//设置顶线条
					var tempLineLeft=tempNodes[m].Top + (tempNodes[m].Height / 2);//var tempLineLeft=tempNodes[m].Left + (tempNodes[m].Width / 2);
					var tempLineHeight=((this.IntervalHeight - this.LineSize) / 2);
					var tempLineTop=tempNodes[m].Left - 	tempLineHeight;//var tempLineTop=tempNodes[m].Top - 	tempLineHeight;
					if(this.DepthGroup[tempNodes[m].Depth].NodeGroups[tempNodes[m].NodeGroupId].length==1){//如果只有一个节点
						var tempBottomLineHeight=parseFloat(tempNodes[m].parentNode.BottomLine.style.width) + this.LineSize;//hxg ?var tempBottomLineHeight=parseFloat(tempNodes[m].parentNode.BottomLine.style.height) + this.LineSize;
						tempNodes[m].parentNode.BottomLine.style.width=(tempLineHeight + tempBottomLineHeight)+"px"; //hxg tempNodes[m].parentNode.BottomLine.style.height=(tempLineHeight + tempBottomLineHeight)+"px";
					}else{
						...
						var temptopLine=new CreateLine(2, tempLineLeft,tempLineTop,  this.LineSize, tempLineHeight, this.LineColor, "LineTop_"+ tempNodes[m].Id);
		...
		for(var n=2; n<=this.Depth; n++){//设置横线条
			...
			for(var m=0; m<tempNodeGroups_.length; m++){
				if(tempNodeGroups_[m].length!=1){
					...
					//var tempLineWidth=tempNodeGroups_[m].Height - (tempNodeGroups_[m][0].Height / 2) + this.LineSize;//var tempLineWidth=tempNodeGroups_[m].Width - (tempNodeGroups_[m][0].Width / 2) + this.LineSize;
					//tempLineWidth-=(tempNodeGroups_[m][tempNodeGroups_[m].length-1].Height / 2);//tempLineWidth-=(tempNodeGroups_[m][tempNodeGroups_[m].length-1].Width / 2);
					var tempLineTop=tempNodeGroups_[m][0].Left - ((this.IntervalHeight - this.LineSize) / 2) - this.LineSize;//var tempLineTop=tempNodeGroups_[m][0].Top - ((this.IntervalHeight - this.LineSize) / 2) - this.LineSize;
					var tempLineLeft=tempNodeGroups_[m][0].Top + (tempNodeGroups_[m][0].Height / 2);//var tempLineLeft=tempNodeGroups_[m][0].Left + (tempNodeGroups_[m][0].Width / 2);
					
					var hxgHeight = tempNodeGroups_[m][tempNodeGroups_[m].length-1].Top -tempNodeGroups_[m][0].Top ;
					...
					var tempGroupLine=new CreateLine(1, tempLineLeft,tempLineTop,  this.LineSize, hxgHeight, this.LineColor, "LineGroup_"+ tempNodeGroups_[m][0].Id);
				    }catch(E){
				        alert(tempLineLeft+","+tempLineTop);   
				    }
				}
			}
		}
	...
	function AmendNodeLeft(Node_, ErrDistance_){//修正错误位置
		Node_.Top=Node_.Top + ErrDistance_;//Node_.Left=Node_.Left + ErrDistance_;
	    ...
	    var maxRight =0;
	    var maxButtom = 0;
	    for(var n=1; n<this.Nodes.length; n++){//取得最大距离
			if(maxRight<this.Nodes[n].Left){
				maxRight=this.Nodes[n].Left;
			}
			if(maxButtom<this.Nodes[n].Top){
				maxButtom=this.Nodes[n].Top;
			}
		} 
		
	    return {"maxButtom":maxButtom  ,"maxRight": maxRight};
	...
	function GetGroupWidthByNode(Node_){//根据群组中任一节点，取得群组宽度
		...
		var tempGroupWidth_=tempNodesGroup_[tempNodesGroup_.length-1].Top - tempNodesGroup_[0].Top;//var tempGroupWidth_=tempNodesGroup_[tempNodesGroup_.length-1].Left - tempNodesGroup_[0].Left;
		tempGroupWidth_+=tempNodesGroup_[tempNodesGroup_.length-1].Height//tempGroupWidth_+=tempNodesGroup_[tempNodesGroup_.length-1].Width
	...
	function SetLeftByDepthNode(DepthNodes_, NodeId, Type){
		...
		if(Type=="LTR"){
			for(var r_=NodeId+1; r_<DepthNodes_.length; r_++){
				if(DepthNodes_[r_].Top!=null){//if(DepthNodes_[r_].Left!=null){
					FindIndex=r_;
					break;
				}
			}
			if(FindIndex==null){
				...
				for(var r_=FindIndex-1; r_>=NodeId; r_--){
					DepthNodes_[r_].Top=DepthNodes_[r_+1].Top - This.IntervalWidth - DepthNodes_[r_].Height;//DepthNodes_[r_].Left=DepthNodes_[r_+1].Left - This.IntervalWidth - DepthNodes_[r_].Width;
				}
			}
		}
		if(Type=="RTL"){
			...
			if(FindIndex==null){
				...
				for(var r_=FindIndex+1; r_<=NodeId; r_++){
					DepthNodes_[r_].Top=DepthNodes_[r_-1].Top + This.IntervalWidth + DepthNodes_[r_-1].Height;//DepthNodes_[r_].Left=DepthNodes_[r_-1].Left + This.IntervalWidth + DepthNodes_[r_-1].Width;
				}
			}
		}
	}
	...
	function GetDepthHeightToRoot(DepthId){//取得距离顶层的高度
		...
		for(var x_=DepthId; x_>=1; x_--){
			tempHeight_+=This.DepthGroup[x_].Width;//tempHeight_+=This.DepthGroup[x_].Height;
		}
		tempHeight_+=This.IntervalHeight * (DepthId - 1);
		tempHeight_-=This.DepthGroup[DepthId].Width;//tempHeight_-=This.DepthGroup[DepthId].Height;
		return tempHeight_;
	}
	...
	function SetLeftPosByChildNode(Node_, ChildNode_){//根据下级节点位置设置节点位置
		...
		if(tempNodeGroups.length==1){
			tempNodeLeft=((ChildNode_.Height / 2) + ChildNode_.Top) - (Node_.Height / 2);//tempNodeLeft=((ChildNode_.Width / 2) + ChildNode_.Left) - (Node_.Width / 2);
		}else{
			tempNodeLeft=GetFirstLeftPos(ChildNode_) + (tempNodeGroups.Height / 2) - (Node_.Height / 2);//tempNodeLeft=GetFirstLeftPos(ChildNode_) + (tempNodeGroups.Width / 2) - (Node_.Width / 2);
		}
		Node_.Top=tempNodeLeft;//Node_.Left=tempNodeLeft;
	} 
	
	function GetFirstLeftPos(Node_){//根据节点位置取得同一级中首个节点位置
		if(Node_.NodeOrderId==0){//Node_为首节点
			return Node_.Top;//return Node_.Left;
		}
		var tempWidth=0;
		for(var k_=0; k_<=Node_.NodeOrderId; k_++){
			var tempGroupsNode=This.DepthGroup[Node_.Depth].NodeGroups[Node_.NodeGroupId][k_];
			tempWidth+=tempGroupsNode.Height;//tempWidth+=tempGroupsNode.Width;
		}
		tempWidth+=(Node_.NodeOrderId * This.IntervalWidth);
		return ((Node_.Top - tempWidth) + (Node_.Height / 2));//return ((Node_.Left - tempWidth) + (Node_.Width / 2));
	}
	...
	function ChildNodesWidth(OrgObj){//取得层宽
		...
		for(var s_=0; s_<OrgObj.Nodes.length; s_++){
			ReWidth+=OrgObj.Nodes[s_].Height;//ReWidth+=OrgObj.Nodes[s_].Width;
		}
	...
	function SetDepthsHeight(){//设置层深高度
		for(var n_=1; n_<=This.Depth; n_++){
			...
			for(var m_=0; m_<tempNodes_.length; m_++){
				if(tempNodes_[m_].Width>MaxHeight){ //if(tempNodes_[m_].Height>MaxHeight){
					MaxHeight=tempNodes_[m_].Width;//MaxHeight=tempNodes_[m_].Height;
				}
			}
			This.DepthGroup[n_].Width=MaxHeight;//This.DepthGroup[n_].Height=MaxHeight;
		}
	}
	...
	function CreateLine(type_, top_, left_, long_, size_, color_, id_){
		...
		if(this.Type==1){ 
			...
			this.Line.style.borderLeftWidth= long_;
			this.Line.style.borderLeftStyle="solid";//this.Line.style.borderTopStyle="solid";
			 
			this.Line.style.width="3px"; //this.Line.style.width=this.Long;
			this.Line.style.height=size_;
		}else{
...
function GetBoxHTML(obj){//取得节点的HTML 
	...
	var TempletStr="<div oncontextmenu='return box_oncontextmenu(this)' onmouseover ='box_onmouseover(this)' onclick='box_onclick(this)' onmouseout ='box_onmouseout(this)' id='" +  
		obj.Id + "' class='box' title='" + title + "' state='" + obj.state + 
		"' nodeType='" + obj.nodeType + "' link='" + obj.link +"' >"+
		 "<table height=40 border=0 cellSpacing='0' cellPadding='0' width=50><tr><td class='box_table_" + boxcolor+"_left'>&nbsp;&nbsp;</td><td nowrap class='box_table_" + boxcolor+"_mid'>" 
		 + caption+ "</td><td class='box_table_" + boxcolor+"_right'>&nbsp;&nbsp;</td></tr></table></div>";
	...
	//是否可编辑
	var editflag = false;
	if(typeof(isCharEdit) !='undefined' && isCharEdit =='true'){
	 	editflag= true
	} 
	
//鼠标在DIV上
function box_onmouseover(div){
	if(div.link !='null'  ){
...
//鼠标移出DIV
function box_onmouseout(div){
	div.style.color='black';
}
//鼠标点击DIV
function box_onclick(div){ 
	if(div.link !='null' ){
		if(editflag ==true){
			showDataDetailForChart(div.link ,div.nodeType);
		}else{
			windowOpenFullScreen("../chartframe/index.jsp?nodeType=" +
				div.nodeType +"&" + div.link);
		}
	}
}
//DIV上点右键
var popMenu = null;
function box_oncontextmenu(div){
	return;
	
	if(editflag ==true){
		if(popMenu == null){
			var model = new AIPopMenuModel();
		    model.addPopMenuItem("act","操作内容",null,null);
		    model.addPopMenuItem("add","新增","act",null);
		    model.addPopMenuItem("copy","复制","act",null);
		    model.addPopMenuItem("del","删除","act",null);
		    model.addSeparator(null);
		    model.addPopMenuItem("disabled","取消",null,null);
...
function initTree(){
    ...
    OrgShows.LineColor='#659fc8';//设置线条颜色
    OrgShows.LineSize=1;  //设置线条大小
    chartSize = OrgShows.Run();
    
	try{ 
        var height =chartSize.maxButtom -0 +100;
        var width =chartSize.maxRight -0 +200; 
        
       	var curFrame = parent.document.getElementById("chart_iframe");
       	
       	if(parseInt(curFrame.style.height)<height){
       		
        	curFrame.style.height = height;
        }
        
        if(parseInt(curFrame.style.width)<width){
        	curFrame.style.width = width;
        }
    }catch(e){}
    
    //如果图片处于编辑状态，则注册刷新方法
    if(typeof(isCharEdit) !='undefined' && isCharEdit =='true'){
    	registerEventWhenDetailChange(reloadChar);
    	registerEventWhenQuoteChange(reloadChar);
    }
}
