...
<body >
		<ai:contractframe id="" title="" contenttype="table"
			...
			<tr>
				...
				<ai:table tableid="workflowTbl"
					setname="com.ai.bce.web.BceWorkflow" needrefresh="true"
					tablemodel="com.ai.appframe2.web.datamodel.MethodModelForService"
					implservice_name="com.ai.bce.configtool.service.interfaces.IConfWorkflowSV"
					implservice_querymethod="getBceWorkflowValues(String cond,int $STARTROWINDEX,int $ENDROWINDEX)"
					implservice_countmethod="getBceWorkflowValuesCount(String cond)"
	...
	<script type="text/javascript">
	 function $(id){
		...
		function getSearchInfo(){ 
    		...
    		getWorkflowTbl().refresh("cond="+cond);
    	}
    	...
    	function getCondtionSql(search_type ,args){
    		if(search_type !='search_all'){
    			...
    			return " or BCE_WORKFLOW_ID like '%" + args 
    				+ "%' or OFFER_ID  like '%" + args 
	...
	function getWorkflowTbl(){
		return g_TableRowSetManager.get("workflowTbl");
	}
	function dbclick(){
		var index = getBatInputFormatTbl().getRow();
		var configId = getBatInputFormatTbl().getValue(index,"CONFIG_ID");
		getBatInputFormatTbl().refresh("cond=CONFIG_ID="+configId);
	}
	function add(){
		var obj = {module_id:<%=moduleId%>};
		var returnVal = window.showModalDialog('addWorkflow.jsp',obj,'dialogHeight=480px;dialogWidth=700px');
		if(returnVal == 1){
			getSearchInfo();
		}	
	}
	function mod(){
		var index = getWorkflowTbl().getRow();
		if(index == -1){
			alert(crm_i18n_msg("BEC0000014"));
			return;
		}
		var workflowId = getWorkflowTbl().getValue(index,"BCE_WORKFLOW_ID");
		var obj = {module_id:<%=moduleId%>,workflow_id:workflowId};
		var returnVal = window.showModalDialog('addWorkflow.jsp',obj,'dialogHeight=480px;dialogWidth=700px');
		if(returnVal == 1){
			getSearchInfo();
		}	
	...
	function del(){
		...
		getWorkflowTbl().deleteRow(index);
		var list = new Array();
	    list.push(getWorkflowTbl());
