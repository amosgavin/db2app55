...
<%
	...
	if(!"".equals(moduleId)){
		...
		if(rulesetType != null && "2".equals(rulesetType)) //后台规则集只能选择java规则
		{
			cond = "(" + cond + ") AND RULE_TYPE=2 ";
		}
...
<script>
	...
	{	
		...
		window.returnValue = table.getValue(table.getRow(),"RULE_ID");
		window.self.close();
	}
	/**
	 * 关闭窗口
	...
	function search(){
		...
		if(cond == "")
		{
			...
			if('<%=rulesetType%>' == '2')
				cond += 'AND RULE_TYPE=<%=rulesetType%>';
		}
		else
		{
