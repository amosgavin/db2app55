...
<body onload="resizeTabToFullWindwo('main_tab')" onresize="resizeTabToFullWindwo('main_tab')">
 	...
 	<ai:tab id="main_tab" height="100%" width="100%" type="H">
 		...
 		%>
 				<ai:tabitem  id="detail" src='<%="detail.jsp?ruleRelateId="+ruleRelateId + "&bceFrameId=" + bceFrameId+ "&module_id=" +module_id%>' 
 		...
 		%>
 				<ai:tabitem id="chart" src='<%="chart_frame.jsp?RELATE_ID="+ruleRelateId+ "&module_id=" +module_id %>' 
		...
		<ai:tabitem id="javarule" src='<%="includeJavaRule.jsp?openType="+openType + "&ruleRelateId= " + ruleRelateId 
				+ "&rulesetId=" + rulesetId+ "&module_id=" +module_id%>' 
			title='<%=LocaleResourceFactory.getResource("BES0000518")%>' 
