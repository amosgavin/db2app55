...
function doUpload(){
	var pid = _ordercomplainFormRowSet.getValue("COMPLAINS_ID");
    ...
    if("" != pid){
        ...
        var aActionUrl="<%=request.getContextPath()%>/business/com.asiainfo.sale.common.web.AttachAction?action=upload&itemId=" + pid + "&itemType=complain" + "&taskId=" + vmTaskId;;
...
function include_reflashAttachTable(){
    ...
    var condition = "itemId=" + pid + "&itemType=complain";
...
function deleteAttachFile(pid)
{
    ...
    if (ud.getValueByName("FLAG") == "Y") {
        _include_fromAttachFileFormRowSet().refresh("itemId=" + pid + "&itemType=complain");
    } else {
