...
<html>
  ...
  <table width="98%" align="center" border="0" cellpadding="0" cellspacing="0">
	<tr>
          ...
          <div id="bycond" style="display: none;">
            ...
            <table width="99%" align="center" border="0" cellpadding="0" cellspacing="0">
	              ...
	              <ai:dbtree_new id="orgStaffTree" datamodel="com.ai.secframe.orgmodel.web.SecOrgCodeTreeModel" 
						multiselect="false" height="600" width="100%" ishaveline="true" 
		...
		<td align="right" valign="top" >
          	 ...
          	 <ai:table tableid="dbTableExistStaff"
                ...
                <ai:button text="删除" id="cancel" onclick="remove()"/></ai:contractitem>
		          	 <ai:table tableid="selectElementTable"
							...
							ondbclick="delThis"
							width="390" height="140" needrefresh="true" footdisplay="true">
                      <ai:col fieldname="OPERATOR_ID" visible="false"/>
                      <ai:col fieldname="STAFF_ID" visible="false"/>
                      <ai:col fieldname="CODE" visible="false"/>
                      <ai:col fieldname="STAFF_NAME" width="130" visible="true"/>
			...
			<div class="area_button">
				<ai:button text="提交" id="cancel" onclick="doReturn()"/>
  ...
  <script type="text/javascript">
    ...
    function searchStaff(){
        ...
        var orgId = "<%=request.getParameter("org_id")%>";
        var nameCond = "";
        var roleIdCond = "";
        var orgIdCond="";
...
<script type="text/javascript">
	...
	var _cancelBtn = g_AIButtonManager.get("cancel");
	//_cancelBtn.setText("添加业支配置NCODE人员");
	_cancelBtn.setText("添加渠道中心审核分配人员");
    ...
    {	
        if(_selectElementsTable.count() > 0)
        {
    		...
    		if(!confirm("确认提交么？本工单可能包含电子渠道承载，需要添加业支配置NCODE审核人，请确认是否需要添加或是否已添加配置NCODE审核人！")){
		        return;
		    }
		   ...
		   if(isSelected){
	           ...
	           if(_sourceElementsTable.count() > 0)
	           {
		            for(var i=0;i<_sourceElementsTable.count();i++)
		            {
                         ...
                         _sourceElementsTable.setRowBgColor(i,"");
		            }
	           }
		   }
		   return;
	   }
	   _sourceElementsTable.setRowBgColor(rowIndex,tmpColor);
	   if(isBreak){
		   return;
	   }
	   if(isSelected){
	...
	{	
	    if( _selectElementsTable.getSelectedRows()!=null&&_selectElementsTable.getSelectedRows().length>0)
	    {
         	...
         	if(_selectElementsTable.count()==0){
         		_cancelBtn.setText("添加渠道审核分配人员");
         		_cancelBtn.setDisabled(false);
         	}else if(_selectElementsTable.count()==1){
         		var staffname = _selectElementsTable.getValue(0, "STAFF_NAME")+"";
         		if(staffname.toString().indexOf("[业支配置NCODE审核人]")>0){
         				_cancelBtn.setText("添加渠道审核分配人员");
		  				_cancelBtn.setDisabled(false);
		  			}else{
		  				_cancelBtn.setText("添加业支配置NCODE审核人");
		  				_cancelBtn.setDisabled(false);
		  		}
	...
	function add()
	{
		var ncount = _selectElementsTable.count();
		  			...
		  			{	var opeType = "";
		  				...
		  				if(ncount==0){
		  					...
		  					if(staffname.toString().indexOf("[业支配置NCODE审核人]")>0){
		  						...
		  						opeType = "[业支配置NCODE审核人]";
		  						var opeIDType ='111111111';
		  					}
                        ...
                        _selectElementsTable.setValue(_selectElementsTable.getRow(),"OPERATOR_ID",_sourceElementsTable.getValue(selRows[i],"OPERATOR_ID")+opeIDType,_sourceElementsTable.getValue(selRows[i],"OPERATOR_ID")+opeIDType);
			            ...
			            _selectElementsTable.setValue(_selectElementsTable.getRow(),"STAFF_NAME",_sourceElementsTable.getValue(selRows[i],"STAFF_NAME")+opeType,_sourceElementsTable.getValue(selRows[i],"STAFF_NAME")+opeType);	            
