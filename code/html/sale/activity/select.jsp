...
<ai:contractframe id="selectSaleOrderframe" contenttype="table" title="营销案查询条件" width="100%" allowcontract="true" frameclosed="fale">
	<ai:contractitem/>
	<ai:dbform formid="selectSaleOrderForm" initial="false"
			setname="com.asiainfo.sale.activity.web.SETSaleOrder">
		<table width="98%" align="center" border="0" cellpadding="1" cellspacing="2">
			<tr>
	           	<td class="td_font">工单号：</td>
	           	<td><ai:dbformfield formid="selectSaleOrderForm" fieldname="ORDER_ID" width="150"/></td>
	           	<td class="td_font">名称：</td>
	           	<td><ai:dbformfield formid="selectSaleOrderForm" fieldname="ORDER_NAME" width="150"/></td>        	
			</tr>
			<tr>
	           	<td class="td_font">申请人：</td>
	           	<td><ai:dbformfield formid="selectSaleOrderForm" fieldname="PROP_NAME" width="150" editable="false"/><img id="selectStaff" border="0" src="<%=request.getContextPath()%>/webframe/images/query.gif" onClick="selectStaff();" style="cursor:hand;"/>&nbsp;<a href="javascript:clsStaff();">清空</a>
	           	    <ai:dbformfield formid="selectSaleOrderForm" fieldname="PRINCIPLE" width="150" editable="false" visible="false"/></td>
	           	<td class="td_font">申请地市：</td>
	           	<td><ai:dbformfield formid="selectSaleOrderForm" fieldname="ORG_ID" width="150"/></td>
			</tr>
			<tr>
	           	<td class="td_font">提交时间大于：</td>
	           	<td><ai:dbformfield formid="selectSaleOrderForm" fieldname="CREATE_DATE" width="170"/></td>
	           	<td class="td_font">提交时间小于：</td>
	           	<td><ai:dbformfield formid="selectSaleOrderForm" fieldname="QUERY_END_DATE" width="170"/></td>
			</tr>
			<tr>
	           	<td class="td_font">工单状态：</td>
	           	<td><ai:dbformfield formid="selectSaleOrderForm" fieldname="STATE" width="150"/>
					<ai:button id="select2table" text="查询" onclick="doWork('select2table()')"/></td>
			</tr>
		</table>
	</ai:dbform>
</ai:contractframe>
<ai:contractframe id="saleMainframe" contenttype="table" title="营销案信息" width="100%" allowcontract="true" frameclosed="fale">
	<ai:contractitem>
	<ai:button id="bt_delSaleMain" text="删除工单" onclick="delSaleMain()"/>&nbsp;&nbsp;
    <ai:button id="bt_cancelWF" text="撤销工单(未被审核)" onclick="cancelWF()"/></ai:contractitem>
 	...
 	<ai:table
		...
		implservice_querymethod="getSaleOrderInfo(String orderId,String orderName, String pinciple, String staffName, String orgId,
			                                      String orgName, String state, String beginTime, String endTime,int $STARTROWINDEX, int $ENDROWINDEX)"
		implservice_countmethod="getSaleOrderCount(String orderId,String orderName, String pinciple, String staffName, String orgId,
		...
		<ai:col title="申请名称" fieldname="ORDER_NAME" width="300" />
...
<div class="area_button">
    <ai:button id="bt_saveSaleMain" text="查看详细" onclick="showSaleDetailInfo()"/>&nbsp;&nbsp;<ai:button id="bt_saveSaleMain" text="通过复制生成新工单" onclick="doWork('cloneSaleInfo()')"/>
</div>
...
var _fromSelectSaleOrderFormRowSet = g_FormRowSetManager.get("selectSaleOrderForm");
...
//var _tableFWHisTableRowSet = g_TableRowSetManager.get("taskAllHisList");

function initOper()
{
	_fromSelectSaleOrderFormRowSet.setValue("PRINCIPLE",g_GetUserInfo().STAFF_ID);
    _fromSelectSaleOrderFormRowSet.setValue("PROP_NAME",g_GetUserInfo().STAFF_NAME);
    //_fromSelectSaleMainFormRowSet.setValue("PROMOTE_DEPART",g_GetUserInfo().ORG_ID,g_GetUserInfo().ORG_ID+"|"+g_GetUserInfo().ORG_NAME);
	//select2table();
    //_fromSelectSaleMainFormRowSet.refreshListBox("IS_SUBMIT","codeType=gdzt2",true);
    //_fromSelectSaleMainFormRowSet.setFocus("IS_SUBMIT");
    _fromSelectSaleOrderFormRowSet.setFocus("ORDER_ID");
    _fromSelectSaleOrderFormRowSet.setValue("CREATE_DATE",getPre1MonTime());
    _fromSelectSaleOrderFormRowSet.setValue("QUERY_END_DATE",getCurrentTime());
}
...
function showSaleDetailInfo(){
    ...
    var editable=false;
    if (_tableSaleOrderInfoTableRowSet.getValue(curRow, "PROP_NAME") == g_GetUserInfo().STAFF_NAME && _tableSaleOrderInfoTableRowSet.getValue(curRow, "STATE") == "1") {
    	editable=true;
    }
    var url = "new.jsp?orderId="+orderId+"&editable="+editable;
