...
<div class="area_button" id="up_file_area">
<table>
...
function doUpload(){
	var pid = _saleMainForm.getValue("MAINID");
    ...
    if("" != pid){
        ...
        var aActionUrl="<%=request.getContextPath()%>/business/com.asiainfo.sale.common.web.AttachAction?action=upload&itemId=" + pid + "&itemType=act" + "&taskId=" + vmTaskId;;
        PostUploadInfo(aFileUploadFormObj,aActionUrl,aXmlAry,aFormXmlParamName,aIsXmlForceFlag);
    } else {
        return alert("请先保存营销活动主要信息主要信息(保存当前)");
    }
}
...
function include_reflashAttachTable(){
    ...
    var condition = "itemId=" + pid + "&itemType=act";
...
function deleteAttachFile(pid)
{
    ...
    if (ud.getValueByName("FLAG") == "Y") {
        _include_fromAttachFileFormRowSet().refresh("itemId=" + pid + "&itemType=act");
    } else {
