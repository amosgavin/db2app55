...
function doUpload(){
	var pid = _fromAccessFormRowSet.getValue("ACCESS_ID");
    ...
    if("" != pid){
        ...
        var aActionUrl="<%=request.getContextPath()%>/business/com.asiainfo.sale.common.web.AttachAction?action=upload&itemId=" + pid + "&itemType=access" + "&taskId=" + vmTaskId;;
...
function include_reflashAttachTable(){
    ...
    var condition = "itemId=" + pid + "&itemType=access";
...
function deleteAttachFile(pid)
{
    ...
    if (ud.getValueByName("FLAG") == "Y") {
        _include_fromAttachFileFormRowSet().refresh("itemId=" + pid + "&itemType=access");
    } else {
