...
function doUpload(){
	var pid = _busiChangeFormRowSet.getValue("BUSI_ID");
    ...
    if("" != pid){
        ...
        var aActionUrl="<%=request.getContextPath()%>/business/com.asiainfo.sale.common.web.AttachAction?action=upload&itemId=" + pid + "&itemType=busiChange" + "&taskId=" + vmTaskId;;
...
function include_reflashAttachTable(){
    ...
    var condition = "itemId=" + pid + "&itemType=busiChange";
...
function deleteAttachFile(pid)
{
    ...
    if (ud.getValueByName("FLAG") == "Y") {
        _include_fromAttachFileFormRowSet().refresh("itemId=" + pid + "&itemType=busiChange");
    } else {
