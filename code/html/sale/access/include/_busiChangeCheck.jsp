...
<%--        if (((taskTag == 'busi007' || taskTag == 'busi010') && messageType == '省业支分派任务') || taskTag == 'busi021') {--%>
    ...
    function doSubmit() {
        ...
        if (taskTag == 'busi007' || taskTag == 'busi010'){
        	if (checkChargeType()) {
	        	 ...
	        	 if ("true" == checkRet.getValueByName("HasTicket") && displaytxt != '同意，并发互联网中心（涉及电子券的业务变更）' && displaytxt != '不同意，退回发起人') {
	        	...
	        	if (displaytxt == '同意，并发互联网中心（涉及电子券的业务变更）') {
	        		if(!confirm("不涉及电子券的资源追加、时间延长不需要发到互联网中心，您确定要提交！")){
		        		 return;
		        	 }
	        	}
	        }
        ...
        //新业务变更流程(template.BusichangeFlow): busi008-省业支分派任务 , busi024-省业支分派任务
        if (taskTag == 'busi008' || taskTag == 'busi024'){
        	if ("false" != result) {
        		if (a_appriseEmp != '') {
