...
<body onload="init()">
<ai:contractframe id="queryIframe" contenttype="table" title="查询条件" width="100%" allowcontract="true" frameclosed="fale">
	<ai:contractitem/>
	<ai:dbform formid="saleStopSellQueryForm" 
		...
		<table width="98%" align="center" border="0" cellpadding="1" cellspacing="2">
			<tr>
	           	...
	           	<td><ai:dbformfield formid="saleStopSellQueryForm" fieldname="MAINID" width="150"/></td>
	           	<td class="td_font">名称：</td>
	           	<td><ai:dbformfield formid="saleStopSellQueryForm" fieldname="APPLY_NAME" width="150"/></td>        	
			</tr>
			<tr>
	           	<td class="td_font">申请人：</td>
	           	<td><ai:dbformfield formid="saleStopSellQueryForm" fieldname="PROP_STAFF" width="150" editable="false"/><img id="selectStaff" border="0" src="<%=request.getContextPath()%>/webframe/images/query.gif" onClick="selectStaff();" style="cursor:hand;"/>&nbsp;<a href="javascript:clsStaff();">清空</a></td>
	           		<ai:dbformfield formid="saleStopSellQueryForm" fieldname="PRINCIPAL" width="150" editable="false" visible="false"/>
	           	<td class="td_font">申请地市：</td>
	           	<td><ai:dbformfield formid="saleStopSellQueryForm" fieldname="ORG_ID" width="150"/></td>
			</tr>
			<tr>
	           	<td class="td_font">提交时间大于：</td>
	           	<td><ai:dbformfield formid="saleStopSellQueryForm" fieldname="PROP_TIME" width="170"/></td>
	           	<td class="td_font">提交时间小于：</td>
	           	<td><ai:dbformfield formid="saleStopSellQueryForm" fieldname="QUERY_END_TIME" width="170"/></td>
			</tr>
			<tr>
	           	<td class="td_font">工单状态：</td>
	           	<td><ai:dbformfield formid="saleStopSellQueryForm" fieldname="STATE" width="150"/>
					<ai:button id="query2table" text="查询" onclick="doWork('query2table()')"/></td>
			</tr>
		</table>
	</ai:dbform>
</ai:contractframe>
<ai:contractframe id="saleStopSellframe" contenttype="table" title="停售信息" width="100%" allowcontract="true" frameclosed="false">
	...
	<ai:table tableid="saleStopSellTab" setname="com.asiainfo.stopSelling.set.SETStopSellM" 
		height="200" multiselect="false" oncellchange="" ondbclick="redirectToDetailInfo"
		oncontextmenu="" needrefresh="true" onrowchange="changeColor" pagesize="15"
...
<ai:loginuser/>
<ai:contractframe id="FWIframe" contenttype="table" title="工单流转详细" width="100%" allowcontract="true" frameclosed="fale">
<ai:contractitem/>
...
<script language="javascript" type="text/javascript">
var saleStopSellQueryForm= g_FormRowSetManager.get("saleStopSellQueryForm");
var saleStopSellTab = g_TableRowSetManager.get("saleStopSellTab");
var taskTab = g_TableRowSetManager.get("taskAllList");
function init(){ 
	
   saleStopSellQueryForm.setValue("PROP_STAFF",g_GetUserInfo().STAFF_NAME);
   saleStopSellQueryForm.setValue("PRINCIPAL",g_GetUserInfo().STAFF_ID);
   saleStopSellQueryForm.setValue("PROP_TIME",getPre1MonTime());
   saleStopSellQueryForm.setValue("QUERY_END_TIME",getCurrentTime());
}

function query2table(){
	
	var mainId = saleStopSellQueryForm.getValue("MAINID");
	var cityId = saleStopSellQueryForm.getValue("ORG_ID");
	var state = saleStopSellQueryForm.getValue("STATE");
	var principal = saleStopSellQueryForm.getValue("PRINCIPAL");
	var applyName = saleStopSellQueryForm.getValue("APPLY_NAME");
	var beginTime = saleStopSellQueryForm.getValue("PROP_TIME");
	var endTime = saleStopSellQueryForm.getValue("QUERY_END_TIME");
	saleStopSellTab.refresh("&mainId="+mainId+"&itemType=sale"+"&cityId="+cityId+"&principal="+principal
...
function redirectToDetailInfo(){
	...
	var curRow = saleStopSellTab.getRow();
    var mainId = saleStopSellTab.getValue(curRow, "MAINID");
    var state = saleStopSellTab.getValue(curRow, "STATE");
    if("" == mainId){
    	alert("请选择要查看的工单！");
    	return;
    }
    var url = "<%=request.getContextPath()%>/stopSelling/sale/saleStopSellShow.jsp?mainId=" + mainId;
...
function del(){
	var curRow = saleStopSellTab.getRow();
    var mainId = saleStopSellTab.getValue(curRow, "MAINID");
    var orderState = saleStopSellTab.getValue(curRow, "STATE");
    if (orderState != '1') {
    	return alert("只能删除保存状态的工单！");
    }
    if (g_GetUserInfo().STAFF_NAME == saleStopSellTab.getValue(curRow, "PROP_STAFF")){
		if(confirm("您确定要删除！")){
			...
			saleStopSellTab.deleteRow(curRow);
			var list = new Array();
...
function back() {
	var curRow = saleStopSellTab.getRow();
    var mainId = saleStopSellTab.getValue(curRow, "MAINID");
    var orgId = saleStopSellTab.getValue(curRow, "ORG_ID");
    var itemType = "saleSellStopCaseT";
    if (orgId.substr(0,2) == "10") {
    	itemType = "saleSellStopCaseP";
    }
    if (mainId == "") {
    	return alert("请选择要回撤的工单！");
    }
    if (saleStopSellTab.getValue(curRow, "STATE") !=2){
    	return alert("只能回撤审核中工单！");
    }
    if (saleStopSellTab.getValue(curRow, "PRINCIPAL") != g_GetUserInfo().STAFF_ID){
    ...
    if(confirm("您确定要回撤！")){
		...
		if ("Y" == rFlag)
		 {
			//saleStopSellTab.refresh("&workflowId=''");
			alert("回退成功！");
		    query2table();
		    return;
		 } else {
...
function selectStaff()
{
    ...
    if(result != null){
        ...
        saleStopSellQueryForm.setValue("PRINCIPAL", operatorId);
        saleStopSellQueryForm.setValue("PROP_STAFF", name); 
    }
} 

function clsStaff(){
	saleStopSellQueryForm.setValue("PRINCIPAL", "");
    saleStopSellQueryForm.setValue("PROP_STAFF", ""); 
}
