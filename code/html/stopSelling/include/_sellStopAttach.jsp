...
function doUpload(){
	var pid = stopSellMainForm.getValue("MAINID");
    ...
    if("" != pid){
        ...
        var aActionUrl="<%=request.getContextPath()%>/business/com.asiainfo.sale.common.web.AttachAction?action=upload&itemId=" + pid + "&itemType=" + attachType + "&taskId=" + vmTaskId;;
...
function include_reflashAttachTable(){
    ...
    var condition = "itemId=" + pid + "&itemType=" + attachType;
...
function deleteAttachFile(pid)
{
    ...
    if (ud.getValueByName("FLAG") == "Y") {
        _include_fromAttachFileFormRowSet().refresh("itemId=" + pid + "&itemType=" + attachType);
    } else {
