...
{
	var _mainId = stopSellMainForm.getValue("MAINID");
    if (null == _mainId || "" == _mainId)
    {
        alert("申请单编号为空");
        return;
    }
    if (("saleSellStopCaseT" == flowType || "saleSellStopCaseP" == flowType) && saleStopSellTab.getTotalRowCount() < 1) {
    	return alert("请填写营销案停售明细信息并保存！");
    } 
    if (("chargeSellStopCaseT" == flowType || "chargeSellStopCaseP" == flowType) && chargeStopSellTab.getTotalRowCount() < 1) {
    	return alert("请填写资费停售明细信息并保存！");
    } 
    ...
    var strUrl = _gModuleName + '/business/com.asiainfo.workflow.util.web.TaskUtilAction?action=createWorkflow&mainId=' + _mainId+"&staffId="+staffId+"&flowType="+flowType+"&result="+result;
    var recode = PostInfo(strUrl);
    if ("Y" == recode.getValueByName("FLAG"))
    {
        alert("提交审核成功");
        if("chargeSellStopCaseT" == flowType || "chargeSellStopCaseP" == flowType){
        	//window.opener.queryChargeMain();
        	var url ="<%=request.getContextPath()%>/stopSelling/charge/chargeStopSellShow.jsp?mainId="+_mainId;
            window.location.href = url;            		
        } 
        if("saleSellStopCaseT" == flowType || "saleSellStopCaseP" == flowType){
        	//window.opener.queryChargeMain();
        	var url ="<%=request.getContextPath()%>/stopSelling/sale/saleStopSellShow.jsp?mainId="+_mainId;
            window.location.href = url;            		
        } 
    } else {
