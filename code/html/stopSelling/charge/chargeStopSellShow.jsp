...
<script language="javascript" type="text/javascript" src="<%=request.getContextPath()%>/jsv2/AIWaitBanner.js"></script>
 ...
 <body onload="init()">
    <ai:contractframe id="chargeStopSellMainframe" contenttype="table" title="资费停售主要信息" width="100%" allowcontract="true" frameclosed="false">
    <ai:contractitem/>
...
<ai:loginuser/>
<%@include file="/stopSelling/include/_chargeInfo.jsp"%>
<%@include file="/stopSelling/include/_sellStopAttach.jsp"%>
<div id="create_wf_div"><%@include file="/stopSelling/include/_sellStopCreateWF.jsp"%></div>
</body>
<script language="javascript" type="text/javascript">
var mainId = "<%=request.getParameter("mainId")%>";
var _orgId = g_GetUserInfo().ORG_ID;
var attachType = "chargeSellStop";
var flowType = "chargeSellStopCaseT";
var templateCode = "template.ChargeStopSellFlow";
var _mainId = mainId;
if (_orgId.substr(0,2) == "10"){
	flowType = "chargeSellStopCaseP";
	templateCode = "template.PChargeStopSellFlow";
}
...
function init(){ 
   ...
   initChargeStopSellDInfo();
   var state =stopSellMainForm.getValue("STATE");
   if (state != '1' || stopSellMainForm.getValue("PRINCIPAL") != g_GetUserInfo().STAFF_ID){
	   stopSellMainForm.setEditSts("false");
	   chargeStopSellForm.setEditSts("false");
	   ...
	   //g_AIButtonManager.get("select").setDisabled("false");
	   ...
	   document.getElementById("create_wf_div").style.display="none";
   } else {
	   initCheckPage();
   }
}
