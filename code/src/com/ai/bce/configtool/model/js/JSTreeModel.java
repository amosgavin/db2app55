 ...
 *
 */
public class JSTreeModel implements DBTreeNewDataModelInterface {

	private static transient Log log = LogFactory.getLog(JSTreeModel.class);

	/** 所有classes目录下面的服务接口 */
	private Map allServices = null;
	
	/** context的文件路径 */
	public static String ctxFilePath = null;
	...
	public void init(ServletRequest req) throws Exception {		
		if (allServices == null) {
			...
			allServices = JSHandler.getServiceMap(file);
		}
	}
	...
	public void getChildren(AIDBTreeNodeInterface parentNode, int arg1) throws Exception {
		try {
			...
			if(parentValue.equals("1")){
				ServiceModel model = JSHandler.rootmodel;
