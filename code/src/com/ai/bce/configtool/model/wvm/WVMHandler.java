...
public class WVMHandler {
	...
	public static Map getServiceMap(File file) throws Exception{
		...
		File[] files = file.listFiles(ModelUtils.getWVMFileFilter());
	...
	protected static void processFile(Map map, ServiceModel parent, String name, File[] files){
		...
		for(int i=0; i<files.length; i++){
			File file = files[i];
			if(file.isDirectory()){			
				File[] childFiles = file.listFiles(ModelUtils.getWVMFileFilter());			
