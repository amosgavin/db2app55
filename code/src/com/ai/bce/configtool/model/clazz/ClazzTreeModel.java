 ...
 *
 */
public class ClazzTreeModel implements DBTreeNewDataModelInterface {

	private static transient Log log = LogFactory.getLog(ClazzTreeModel.class);
	...
	public AIDBTreeNodeInterface getRootNode() {
		...
		try {
			...
			node.setValue("D");
			node.setUserObj("1");
			node.setLabel(LocaleResourceFactory.getResource("BES0000815"));//服务列表
		} catch (Exception ex) {
			log.error(ex);
		}
		return node;
	...
	public void init(ServletRequest req) throws Exception {		
		if (allServices == null) {
			...
			allServices = ClazzHandler.getServiceMap(file);
		}
	}
	...
	public void getChildren(AIDBTreeNodeInterface parentNode, int arg1) throws Exception {
		try {
			...
			if(parentValue.equals("1")){
				ServiceModel model = ClazzHandler.rootmodel;
