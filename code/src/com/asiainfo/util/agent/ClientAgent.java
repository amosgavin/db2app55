...
/*******************************************************************************
 * author:zxh 功能说明：工作流接口封装，提供了启动工作流和提交功能，补充了comframe无法判断角色权限的问题， 提供了角色权限判断功能
 ******************************************************************************/

public class ClientAgent {

	/***************************************************************************
	 * 功能说明：创建流程实例 参数说明： flowCode 模板代码 staffId 操作员ID objectTypeId
	 * 工单类型（如"salecase"表示营销活动申请） objectId 工单ID result 流程结果（在创建流程时不需要结果，直接传空值即可）
	...
	public static boolean createWorkflow(String flowCode, String staffId,
		...
		ISendSmsSV sendsms = (ISendSmsSV)ServiceFactory.getService(ISendSmsSV.class);
		try {
			CenterFactory.setDirectCenterInfo(new CenterInfo("0", vorgId));
			String wid = ComframeClient.createWorkflow(flowCode, staffId, objectTypeId,
					objectId, aVars, null, notes);
			CenterFactory.setCenterInfoEmpty();
			sendsms.sendSms(objectTypeId, objectId, staffId, staffId);
		} catch (Exception e) {
			e.printStackTrace();
			rs = false;
			return rs;
	 ...
	 * startTime 创建时间 notes 流程备注
	 **************************************************************************/
	public static boolean createWorkflow(String flowCode, String staffId,String nextStaffId,
			String objectTypeId, String objectId, String result, String notes,
			String orgId) {
		boolean rs = true;
		System.setProperty(Constant.S_COMFRAME_DEV_NAME, "CM");
		...
		try {
			...
			String wid=ComframeClient.createWorkflow(flowCode, staffId, objectTypeId,
					objectId, aVars, null, notes);
			CenterFactory.setCenterInfoEmpty();
			sendsms.sendSms(objectTypeId, objectId, staffId, nextStaffId);
		} catch (Exception e) {
			e.printStackTrace();
			rs = false;
			return rs;
	 ...
	 * reason 说明
	 **************************************************************************/
	public static boolean assignReason(String role, String staffId,
			String objectTypeId, String objectId, String result,
			String reason, String staffStr) {
		...
		try {
			...
			rs = ComframeClient.finishUserTask(taskId, staffId, null,
					reason, aVars);
			sendSms.sendSms(taskId, staffId, staffStr);
	...
	public static boolean assignTask(String role, String staffId,
		...
		try {
			...
			sendSms.sendSms(taskId, staffId, staffNext);
	...
	public static boolean backTask(String taskId, String staffId,
		...
		HashMap<String, String> parameter = new HashMap<String, String>();
		parameter.put("workflow_id", curTask.getWorkflowId());
		String condition = "workflow_id=:workflow_id and state=5";

		TaskInfo childTaskArray[] = ComframeClient.getTaskInfos("HB",
				condition, parameter, 0, 1000);
		if(childTaskArray.length>1){
			return rs;
		}
		TaskInfo childTask = childTaskArray[0];

		//while (!childTask.getTasktype().equals("user")&&(childTask.getTasktype().equals("user")&&childTask.getState()==6)) {
		//	if(!childTask.getTasktype().equals("sign")&&(childTask.getTasktype().equals("sign")&&childTask.getState()==6)){
		//		parameter.put("taskId", childTask.getTaskId());
		//		childTaskArray = ComframeClient.getTaskInfos("HB", condition,
		//				parameter, 0, 1000);
		//		if(childTaskArray.length>1){
		//			for(int i=0;i<childTaskArray.length;i++){
		//				if(childTaskArray[i].getState()==5) childTask=childTaskArray[i];
		//			}
		//		}else{
	...
	public static String reAuthorizeTask(String taskId,
			...
			Exception {
		String rs;
		rs = ComframeClient.reAuthorizeTask(taskId, staffId, "-1",
				authorizeStaffId);
		return rs;
	}
